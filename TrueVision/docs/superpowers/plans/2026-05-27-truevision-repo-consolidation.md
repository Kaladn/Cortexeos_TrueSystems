...
**Architecture:** Keep the proven runtime packages and scripts, then make the repo shape explicit: runtime code in package roots, operator entrypoints in `scripts/`, ignored runtime data in `storage/`, bulky proof media in `outputs/`, and TrueCore handoff candidates parked. Move only after inventory, tests, and compatibility shims exist.
...
historical_truecore_data/  ~178 MB, old imported data
...
transfer_to_truecore/
  parked agent candidates only
  no active TrueCore integration by folder presence
```
...
```powershell
git add docs/ACTIVE_TOOL_SURFACE.md docs/TRUEVISION_LOCAL_PRODUCT_MAP.md docs/TRUEVISION_WORKER_RACK_CONTRACT.md scripts/truevision_worker_forge.py tests/test_worker_forge.py tests/test_worker_forge_script.py transfer_to_truecore truevision_runtime/worker_forge.py
git commit -m "Add TrueVision worker forge and TrueCore agent candidates"
```

Expected:

```text
[main <hash>] Add TrueVision worker forge and TrueCore agent candidates
```
...
LANES = {
    ...
    "parked_truecore_transfer": ["transfer_to_truecore"],
    "legacy_or_external": ["historical_truecore_data", "connected_artifacts", "reports"],
}
...
../TrueAudio/trueaudio_runtime/    active audio-state runtime package
...
transfer_to_truecore/ parked TrueCore agent candidates
```
...
Do not move `../TrueAudio/trueaudio_runtime/` or `trueframegen/` yet. They are working packages with tests and scripts.

Do not delete `outputs/` yet. First produce a cleanup dry-run manifest, then choose what to remove.

Do not move TrueCore agents into TrueCore. `transfer_to_truecore/` remains parked.
