...
## TrueCore Agent Boundary

Only agents transfer to TrueCore.
...
TrueCore receives only coordinating/reasoning/gating agents.
```

TrueCore agent candidates must use TrueCore's exact live-agent manifest
...
no worker migration into TrueCore
no policy authority in organ workers
no promotion-ready claim before TrueCore tests pass
```
