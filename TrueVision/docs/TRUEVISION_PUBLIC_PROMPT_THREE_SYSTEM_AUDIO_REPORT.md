...
Scope: TrueVision and audio-state items only. AnchorWorks and TrueCore are included only where this TrueVision repo directly documents the relationship.
...
file: README.md
line/range: 12-14
evidence: TrueVision records state. AnchorWorks interprets meaning. TrueCore proves safety.
evidence_level: DIRECT_DOC
runtime_status: DOCS_ONLY
```
...
The TrueCore boundary document keeps TrueCore as gate and receipt layer, not a middle brain:

```text
file: docs/TRUEVISION_TRUECORE_SAFETY_BOUNDARY.md
line/range: 8-9, 21-27, 42-44
evidence: TrueCore is not the middleman brain; it is guardrail, policy gate, receipt layer, and runtime safety wrapper; it may verify manifests/receipts, gate retention, and preserve suspicious windows; TrueVision does not become TrueCore.
evidence_level: DIRECT_DOC
runtime_status: DOCS_ONLY
```
...
```text
file: ../TrueAudio/trueaudio_runtime/__init__.py
line/range: 1-4
...
```text
file: ../TrueAudio/trueaudio_runtime/replayable.py
...
```text
file: ../TrueAudio/trueaudio_runtime/replay.py
...
- TrueCore may verify manifests, receipts, policy, and retention boundaries.
...
6. TrueCore receipt/safety boundary
...
evidence: This report did not inspect AnchorWorks or TrueCore repositories. Claims about those systems are limited to TrueVision repo docs.
