...
| TrueAudio runtime | `../TrueAudio/trueaudio_runtime/` | exists | Logs derived pre-output audio state from decoded PCM. |
| TrueAudio CLI | `../TrueAudio/scripts/trueaudio_log_pre_sound.py` | exists | Command-line entry point for TrueAudio state logging. |
| TrueAudio machine CLI | `../TrueAudio/scripts/trueaudio_log_machine_pre_sound.py` | exists | Command-line entry point for local machine output-mix logging. |
| TrueAudio state replay CLI | `../TrueAudio/scripts/trueaudio_replay_state.py` | exists | Renders deterministic WAV sonification from TrueAudio state logs. |
| TrueAudio file replayable CLI | `../TrueAudio/scripts/trueaudio_log_file_replayable.py` | exists | Builds replayable spectral state from a source audio file through FFmpeg decode. |
| TrueAudio replayable capture CLI | `../TrueAudio/scripts/trueaudio_log_machine_replayable.py` | exists | Captures machine output as replayable spectral state, not raw PCM. |
| TrueAudio replayable replay CLI | `../TrueAudio/scripts/trueaudio_replay_replayable.py` | exists | Reconstructs close WAV audio from replayable spectral state. |
| TrueSpeech detection CLI | `../TrueSpeech/scripts/truespeech_detect_segments.py` | exists | Detects speech/background segments from replayable TrueAudio state. |
| TrueSpeech lyric alignment CLI | `../TrueSpeech/scripts/truespeech_align_lyrics_candidate.py` | exists | Aligns provided lyrics to speech-state windows as candidates, not ASR truth. |
...
TrueCore = later policy, approvals, retention, receipts
