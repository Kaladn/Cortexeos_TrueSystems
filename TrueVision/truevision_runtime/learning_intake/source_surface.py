...
TRUEVISION_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_CAPTURE_OUTPUT_ROOT = str(TRUEVISION_ROOT / "storage" / "artifacts" / "source_surface_captures")
DEFAULT_NATIVE_CAPTURE = str(
    TRUEVISION_ROOT / "native" / "truevision_capture_rs" / "target" / "release" / "truevision_capture_rs"
)
...
def build_source_surface_multi_sample_plan(
    *,
    ...
    output_root: str = DEFAULT_CAPTURE_OUTPUT_ROOT,
    native_capture_binary: str = DEFAULT_NATIVE_CAPTURE,
        ...
        samples.append(
            {
                **window,
                ...
                "native_capture_command": _native_capture_command(
                    capture_binary=native_capture_binary,
                    duration_seconds=capture_duration,
                    fps=fps_value,
                    resolution=capture_resolution,
                    grid=capture_grid,
...
def write_source_surface_capture_plan(args: dict[str, Any], *, storage_root: Path) -> dict[str, Any]:
    plan = build_source_surface_capture_plan(
        ...
        native_capture_binary=str(
            args.get("native_capture_binary")
            or DEFAULT_NATIVE_CAPTURE
        ),
