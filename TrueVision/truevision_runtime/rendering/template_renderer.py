...
def memory_snapshot() -> dict[str, Any]:
    ...
    try:
        ...
        def kibibytes(key: str) -> int | None:
            value = status.get(key, "").split()
            return int(value[0]) * 1024 if value and value[0].isdigit() else None
        snapshot.update(
            {
                "working_set_bytes": kibibytes("VmRSS"),
                "peak_working_set_bytes": kibibytes("VmHWM"),
                "pagefile_usage_bytes": kibibytes("VmSwap"),
                "private_usage_bytes": kibibytes("RssAnon"),
            }
        )
    except OSError:
        pass
...
def capture_hardware() -> dict[str, Any]:
    ...
    try:
        ...
        def mem_bytes(key: str) -> int | None:
            value = meminfo.get(key, "").split()
            return int(value[0]) * 1024 if value and value[0].isdigit() else None
        ram = {"total_physical_bytes": mem_bytes("MemTotal"), "available_physical_bytes": mem_bytes("MemAvailable")}
    except OSError:
        pass
    try:
        completed = subprocess.run(["lspci", "-mm"], check=False, capture_output=True, text=True, timeout=5)
        gpus = [{"description": line} for line in completed.stdout.splitlines() if "VGA compatible controller" in line or "3D controller" in line]
