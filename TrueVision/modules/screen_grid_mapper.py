...
class ScreenGridMapper:
    ...
    def __init__(
        ...
        Args:
            ...
            capture_region: Optional absolute screen crop (left, top, width, height)
        """
        ...
        # MSS is needed only by the live capture boundary, not by state readers.
        try:
            import mss
        except ModuleNotFoundError as exc:
            raise RuntimeError("live screen capture requires the optional Linux mss package") from exc
        self.sct = mss.mss()
        self.monitor_config = dict(self.sct.monitors[monitor + 1])  # 0 = primary monitor
        ...
        if self.grid_rows % self.block_rows != 0 or self.grid_cols % self.block_cols != 0:
            raise ValueError("grid_size must divide evenly into block_size")
        ...
        # Cell dimensions in the actual frame that will be analyzed.
        self.effective_width = capture_resolution[0] if capture_resolution else self.screen_width
        self.effective_height = capture_resolution[1] if capture_resolution else self.screen_height
        self.cell_width = self.effective_width / self.grid_cols
        self.cell_height = self.effective_height / self.grid_rows
        ...
        self.resonance = ScreenResonanceState(
            grid_size=(self.block_rows, self.block_cols),  # Operates on aspect-aware blocks
            ema_alpha_fast=0.3,
            ema_alpha_slow=0.1
        )
        ...
        print(f"  Source region: {self.screen_width}×{self.screen_height}")
        print(f"  Analyzed frame: {self.effective_width}×{self.effective_height}")
    ...
    def extract_features(self) -> Dict[str, np.ndarray]:
        ...
        return {
            ...
            'capture_geometry': {
                'source_width': self.screen_width,
                'source_height': self.screen_height,
