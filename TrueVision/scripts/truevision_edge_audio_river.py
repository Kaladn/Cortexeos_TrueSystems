...
DEFAULT_AUDIO = Path("inputs/edge_of_the_world_audio")
DEFAULT_LYRICS = Path("inputs/edge_of_the_world_lyrics.txt")
...
def capture_hardware() -> dict[str, Any]:
    ...
    try:
        ...
        ram = {
            "total_physical_bytes": int(values["MemTotal"].split()[0]) * 1024,
            "available_physical_bytes": int(values["MemAvailable"].split()[0]) * 1024,
        }
