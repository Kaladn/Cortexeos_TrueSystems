...
def find_browser() -> Path:
    resolved = shutil.which("chromium") or shutil.which("google-chrome")
    if resolved:
        return Path(resolved)
    raise RuntimeError("Chromium was not found for headless panel export")
...
def system_specs(cwd: Path) -> dict:
    ...
    for line in Path("/proc/meminfo").read_text(encoding="utf-8", errors="replace").splitlines():
        if line.startswith("MemTotal:"):
            ...
            break
    lspci = run(["lspci"], cwd=cwd, allow_fail=True)
    gpu_names = [line.split(": ", 1)[-1] for line in lspci.stdout.splitlines() if "VGA compatible controller" in line or "3D controller" in line]
    return {
        "platform": platform.platform(),
        "python": platform.python_version(),
        "cpu": {"Name": cpu_name, "NumberOfLogicalProcessors": os.cpu_count()},
        "memory": {"TotalPhysicalMemory": memory_kib * 1024},
        "gpu": gpu_names,
