...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="TrueVision local mini-TrueCore worker/tool forge. Manifest-only; no worker execution."
    )
    sub = parser.add_subparsers(dest="command", required=True)

    inventory = sub.add_parser("inventory", help="Inventory local tools, workers, and staged TrueCore agent candidates.")
    ...
    inventory.add_argument(
        ...
        type=Path,
        default=ROOT / "transfer_to_truecore" / "truevision_agent_candidates",
    )
