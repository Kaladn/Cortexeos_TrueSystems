...
EDGE_EXE = Path(shutil.which("chromium") or shutil.which("google-chrome") or "/usr/bin/chromium")
NATIVE_CAPTURE_BINARY = ROOT / "native" / "truevision_capture_rs" / "target" / "release" / "truevision_capture_rs"
...
def _start_browser(profile_dir: Path, url: str, *, devtools_port: int) -> subprocess.Popen[Any]:
    ...
    for source in queue["sources"]:
        for sample in source["samples"]:
            ...
            try:
                ...
                if first_navigation:
                    browser_process = _start_browser(browser_profile, url, devtools_port=devtools_port)
...
def main() -> int:
    ...
    run_root = Path(args.run_root) if args.run_root else ROOT / "storage" / "library" / "youtube_learning_intake_pilot" / run_id
    ...
    queue = build_batch_queue(
        ...
        native_capture_binary=str(NATIVE_CAPTURE_BINARY),
    )
