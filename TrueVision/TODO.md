...
- [x] Add local product map, active tool surface, parked experiment map, and TrueMem/SC boundary contracts.
...
- [x] Replace the obsolete Windows WASAPI backend with the TrueAudio sibling's Linux PipeWire/Pulse monitor capture.
...
- [ ] Open the other workspace in TrueCore first.
- [ ] Inventory what is actually on the TrueCore / AnchorWorks / TrueVision workbench.
- [ ] Lock the cross-system harness shape before adding new features.
- [ ] Prove AnchorWorks, TrueCore, and TrueVision health checks separately.
...
TrueCore = safety, agents, logging, policy, system substrate
