# AWEAR/AWRAG Recognition + Evidence System — Direction Document
...
For now, treat AWEAR/AWRAG as the same general retrieval/evidence family unless a later specification explicitly separates them.
...
# 4. The chat surface does not own AWEAR

Chat-Chain and AWEAR/AWRAG are separate systems.
...
Chat-Chain must not implement AWEAR internally.

AWEAR must not require Chat-Chain to know its internal retrieval algorithms.
...
message
    ...
    producer = human | codex | chatgpt | awear | other
...
AWEAR symbolic layer = recognition/index structure
```
...
AWEAR/AWRAG should be comfortable reporting absence.
