# Forest AI — Deterministic Plugin Integration Guide
...
A Forest AI plugin is a Python package that:
...
**File**: `bridges/forest_bridge_server.py`
...
**LakeSpeak reference**: `forest_bridge_server.py:1493-1499`
...
**LakeSpeak reference**: `forest_bridge_server.py:475-651`
...
**File**: `forest.config.json` (workspace root)
**Your config module**: `{plugin}/config.py`

### Config pattern

```python
"""Plugin configuration — reads from forest.config.json."""
...
def load_config(config_path: Path = None) -> Dict[str, Any]:
    ...
    if config_path is None:
        try:
            ...
            config_path = SOURCE_ROOT / "forest.config.json"
        except ImportError:
...
**Rules:**
- Your key in forest.config.json is your plugin directory name
...
- Never modify forest.config.json programmatically
...
Every plugin gets a directory tree under `%LOCALAPPDATA%\ForestAI\`.

### Add your paths

```python
# ── {PluginName} ──────────────────────────────────────────────
{PLUGIN}_DIR         = FOREST_DATA_ROOT / "{plugin}"
...
**Rules:**
- Your top-level directory is `FOREST_DATA_ROOT / "{plugin}"`
...
def _zone_root(zone: WriteZone) -> Path:
    _map = {
        # ... existing ...
        WriteZone.{PLUGIN}_INDEX:   FOREST_DATA_ROOT / "{plugin}" / "index",
        WriteZone.{PLUGIN}_EVENTS:  FOREST_DATA_ROOT / "{plugin}" / "events",
        WriteZone.{PLUGIN}_EVAL:    FOREST_DATA_ROOT / "{plugin}" / "eval",
    }
    return _map[zone]
```
...
**File**: `ui/forest_ai_production.js`
...
**LakeSpeak reference**: `ui/forest_ai_production.js:852-880, 4155-4200, 4215`
...
### What your plugin CAN import from Forest AI
...
| `bridges.forest_bridge` | Lazy (`try/except`) | Lexicon bridge access |
...
| `bridges.forest_bridge_server` | Circular: bridge imports plugin |
...
1. `bridges/forest_bridge_server.py` — mounts your router
2. `bridges/forest_bridge_server.py` — calls `get_engine()` in mode handler
...
- [ ] Add router mount try/import to `bridges/forest_bridge_server.py`
...
- [ ] Add `"{plugin}": { ... }` section to forest.config.json
