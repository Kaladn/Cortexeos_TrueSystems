...
class _MissingPluginGateway:
    def get_lakespeak_engine(self) -> Any:
...
def test_grounded_returns_not_available_when_plugin_missing() -> None:
    async def _run() -> None:
        ...
        svc = GroundedChatService(
            ...
            plugin_gateway=_MissingPluginGateway(),  # type: ignore[arg-type]
        )
        ...
        assert "LakeSpeak plugin not installed" in resp.response
