"""Chat Packs API Router -- FastAPI endpoints.
...
from core.chat_packs.api.models import (
    LessonViewResponse,
    PackDetailResponse,
    PackInstallRequest,
    PackInstallResponse,
...
def get_engine():
    ...
    if _engine is None:
        with _engine_lock:
            if _engine is None:
                from core.chat_packs.core.engine import ChatPackEngine
...
def _version() -> str:
    try:
        from core.chat_packs import VERSION
        return VERSION
    except (ImportError, AttributeError):
        return "0.0.0"


def _pick_install_folder(initial_dir: Path | None = None) -> str:
    try:
...
async def cp_status():
    """Service status and content stats."""
    try:
        engine = get_engine()
        engine.rescan()
...
@router.post("/pick-install-folder", response_model=PackPickerResponse)
async def cp_pick_install_folder():
    """Open a native folder picker and return the selected source path."""
    try:
        engine = get_engine()
        initial_dir = None
        allowed_roots = engine._config.get("allowed_install_roots", [])
        if allowed_roots:
            initial_dir = Path(str(allowed_roots[0])).expanduser()
        elif os.name == "nt":
            initial_dir = Path.home()
        selected = _pick_install_folder(initial_dir=initial_dir)
        return PackPickerResponse(selected_path=selected, cancelled=(selected == ""))
    except Exception as e:
        logger.error("Chat Packs picker error: %s", e, exc_info=True)
        return PackPickerResponse(error={"type": "picker_error", "message": str(e)})
