...
Follows the same config pattern as other core services:
...
_DEPRECATED_STAGE_TYPES = frozenset({"reasoning_engine"})
...
def _normalize_profile(profile: dict) -> dict:
    """Strip deprecated routing stages from a profile copy."""
    ...
    if isinstance(pipeline, list):
        normalized["pipeline"] = [
            ...
            if stage.get("type") not in _DEPRECATED_STAGE_TYPES
        ]
        for stage in normalized["pipeline"]:
            if stage.get("type") != "plugin_chain":
                continue
...
def load_named_profile(name: str) -> dict | None:
    ...
    if not fp.exists():
        return None
    try:
        return _normalize_profile(json.loads(fp.read_text(encoding="utf-8")))
    except (json.JSONDecodeError, OSError):
        return None
