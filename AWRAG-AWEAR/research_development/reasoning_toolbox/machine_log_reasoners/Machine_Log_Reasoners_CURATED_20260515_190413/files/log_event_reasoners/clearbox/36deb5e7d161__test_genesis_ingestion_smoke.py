...
# Requires Bridge running on 127.0.0.1:5050 with genesis_cite mounted at /api/genesis/*
#
...
#   GENESIS_EXPECTED_BLOCKS     default: 68
#   GENESIS_EXPECTED_COMMIT     default: bf8e3a1  (set "" to disable commit check)
...
EXPECTED_BLOCKS = int(os.environ.get("GENESIS_EXPECTED_BLOCKS", "68"))
EXPECTED_COMMIT_PREFIX = os.environ.get("GENESIS_EXPECTED_COMMIT", "bf8e3a1")
...
def _genesis_available() -> bool:
    """Return True if the genesis plugin is mounted and healthy on the bridge."""
    try:
...
# Module-level skip: if bridge isn't up or genesis plugin isn't mounted, skip all.
if not _genesis_available():
    pytest.skip(
        f"Bridge genesis plugin not available at {HOST}:{PORT}/api/genesis — "
        "start the bridge and ensure genesis_cite is mounted",
...
def _req(method: str, path: str, payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if resp.status >= 400:
        raise AssertionError(f"{method} {path} -> HTTP {resp.status}\n{raw}")
...
def test_direct_spot_checks():
    ...
    for tag in ("G-0017", "G-0028", "G-0058", "G-0068"):
...
def test_missing_tag_not_found_contract():
    resp = _req("POST", "/api/genesis/cite", {"mode": "direct", "tag": "G-9999", "include_body": True})
    assert resp.get("ok") is False, f"Expected ok:false, got: {resp}"
    assert resp.get("error") == "NOT_FOUND", f"Expected NOT_FOUND, got: {resp.get('error')}"
...
def test_path_injection_rejected():
    for bad in ("../etc/passwd", "G-0001/evil", "G-0001\\\\evil"):
        resp = _req("POST", "/api/genesis/cite", {"mode": "direct", "tag": bad})
        assert resp.get("ok") is False, f"Expected rejection for tag={bad!r}, got: {resp}"
