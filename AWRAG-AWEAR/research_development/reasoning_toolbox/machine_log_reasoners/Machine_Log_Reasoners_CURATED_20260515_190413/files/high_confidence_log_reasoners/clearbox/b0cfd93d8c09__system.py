...
def make_router(
    ...
    metrics:             _METRICS dict from create_app (shared reference)
    mounted_plugins:     _mounted_plugins set from create_app (shared reference)
    background_tasks:    _background_tasks set from create_app (shared reference)
    task_done_cb:        _task_done callback for fire-and-forget tasks
    base_dir:            BASE_DIR path constant
    plugin_runtime:      PluginRuntimeService for dynamic plugin discovery
    mounted_plugin_routes: mounted plugin id -> mounted route prefixes
    """
    router = APIRouter()
