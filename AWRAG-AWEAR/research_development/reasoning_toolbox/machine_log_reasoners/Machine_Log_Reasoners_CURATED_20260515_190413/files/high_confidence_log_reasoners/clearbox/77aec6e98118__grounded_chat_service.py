...
class GroundedChatService:
    ...
    def __init__(self, *, state: BridgeState, lakespeak_service: LakeSpeakService | None = None) -> None:
        self.state = state
        self.lakespeak_service = lakespeak_service or LakeSpeakService()
    ...
    async def execute(
        ...
        *,
        ...
        try:
            engine = self.lakespeak_service.get_engine()
            ...
            route.record(rctx, "lakespeak", "lakespeak", True, ms=int((time.time() - t0_retrieval) * 1000))
            ...
            return ChatSendResponse(
                ...
                response="LakeSpeak is unavailable.",
                reasoning_trace={"routing": route.telemetry_block(rctx)},
                error={"type": "not_available", "message": "LakeSpeak is unavailable"},
            )
        except Exception as e:
