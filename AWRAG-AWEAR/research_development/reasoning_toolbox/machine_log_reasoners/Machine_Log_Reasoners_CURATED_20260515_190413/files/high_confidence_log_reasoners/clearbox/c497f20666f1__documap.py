...
def make_router(
    ...
    @router.post("/api/documap/delete")
    async def documap_delete(body: dict):
        """Delete a document from the data lake.

        Removes the receipt directory (chunks.jsonl, anchors.json, relations.json,
        refs.json). Cleans search indexes. Does NOT touch evidence.bin or lexicon.

        Body: {"receipt_id": "rcpt_..."}
        """
        return await query_service.delete_receipt(body.get("receipt_id", ""))
