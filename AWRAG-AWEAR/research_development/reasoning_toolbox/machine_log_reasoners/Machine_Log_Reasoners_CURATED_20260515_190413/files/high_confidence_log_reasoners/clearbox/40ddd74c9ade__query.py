...
Pipeline:
  1. Extract anchors from query -> extract lexicon presence/absence
...
from ..storage import DEFAULT_CORPUS_ID, iter_receipt_dirs, normalize_corpus_id, resolve_receipt_dir
from ..text.normalize import extract_anchors as _extract_anchors

logger = logging.getLogger(__name__)

# ── Binary citation store (lazy-loaded) ───────────────────────
_cite_reader = None
_cite_appender = None


def _get_cite_reader():
    """Lazy-load binary citation reader. Returns None if store doesn't exist."""
    global _cite_reader
    if _cite_reader is not None:
        return _cite_reader
    try:
        from security.data_paths import DATA_CITATIONS_BIN_DIR
        from bridges.binary_store import CitationStoreReader
        bin_path = DATA_CITATIONS_BIN_DIR / "citations.bin"
        if not bin_path.exists():
            return None
        _cite_reader = CitationStoreReader(DATA_CITATIONS_BIN_DIR)
        _cite_reader.load()
        return _cite_reader
    except Exception as e:
        logger.debug("Binary citation reader not available: %s", e)
        return None
...
class LakeSpeakEngine:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self._bridge = None     # Lazy: ClearboxLexiconBridge
    ...
    def _ensure_bridge(self):
        """Lazy-load Clearbox lexicon bridge (best-effort)."""
        if not self._bridge_loaded:
            ...
            try:
                ...
                config_path = CLEARBOX_CONFIG_PATH
                ...
                logger.debug("Clearbox lexicon bridge not available")
            except Exception as e:
    ...
    def _extract_snippet(
        ...
        # Extract query terms (skip short tokens)
        terms = [t for t in _extract_anchors(query) if len(t) >= 3]
    ...
    def _build_citations(
        ...
        for sc in scored_chunks:
            ...
            record = {
                ...
                "corpus_id": sc.corpus_id or cd.get("corpus_id", DEFAULT_CORPUS_ID),
            ...
            try:
                ...
                # Dedup: check binary store before creating new record
                reader = _get_cite_reader()
                existing = reader.resolve(coord) if reader else None
                if existing:
                    record.update({
                        "cite_id": existing.cite_id,
                        "created_at_utc": existing.to_dict()["created_at_utc"],
                        "unresolved": existing.unresolved,
                        "canonical": existing.coord,
                    })
                else:
                    ...
                    # Persist new citation to binary store
                    appender = _get_cite_appender()
                    if appender:
                        try:
                            from bridges.binary_store import CitationRecord
                            bin_rec = CitationRecord.from_json(intake_record)
                            bin_rec.receipt_id = sc.receipt_id
    ...
    def _load_chunk_data(
        ...
        needed_by_receipt: Dict[tuple[str, str], Set[str]] = {}
        for sc in scored_chunks:
            if sc.chunk_id not in data:
                cid = sc.corpus_id or DEFAULT_CORPUS_ID
                needed_by_receipt.setdefault((cid, sc.receipt_id), set()).add(sc.chunk_id)

        for (corpus_id, receipt_id), needed_ids in needed_by_receipt.items():
            resolved = resolve_receipt_dir(receipt_id, corpus_id=corpus_id)
            if not resolved:
                continue
            resolved_corpus_id, receipt_dir = resolved
            ...
            try:
                for line in chunks_file.read_text(encoding="utf-8").splitlines():
                    ...
                    if cid in needed_ids:
                        data[cid] = {
                            "text": chunk.get("text", ""),
                            "corpus_id": chunk.get("corpus_id", resolved_corpus_id),
                            "receipt_id": chunk.get("receipt_id", receipt_id),
                            "source_path": chunk.get("source_path"),
                            "lake_path": str(chunks_file.resolve()),
    ...
    def _load_chunk_anchors(
        ...
        for sc in scored_chunks:
            cache_key = f"{sc.corpus_id or DEFAULT_CORPUS_ID}:{sc.receipt_id}"
            if cache_key in loaded_receipts:
                continue
            loaded_receipts.add(cache_key)

            resolved = resolve_receipt_dir(sc.receipt_id, corpus_id=sc.corpus_id)
            if not resolved:
                continue
            resolved_corpus_id, receipt_dir = resolved
            ...
            try:
                ...
                for item in data:
                    ...
                    anchors_map[chunk_id] = ChunkAnchors(
                        ...
                        corpus_id=item.get("corpus_id", resolved_corpus_id),
    ...
    def query(
        ...
        from .grounding_policy import apply_policy
        from ..index.anchor_reranker import extract_query_anchors, rerank
        from ..events.hashing import (
        ...
        scoped_corpora = None
        if corpora:
            scoped_corpora = {normalize_corpus_id(c) for c in corpora if str(c).strip()}
            if not scoped_corpora:
                scoped_corpora = None
        ...
        bm25_results = bm25_index.query(query, topk=bm25_topk, corpora=scoped_corpora)
        ...
        if dense_index is not None:
            try:
                ...
                dense_results = dense_index.query(query, topk=dense_topk, corpora=scoped_corpora)
            except Exception as e:
                logger.warning("Dense retrieval failed, falling back to BM25-only: %s", e)

        if dense_results:
            from ..index.hybrid import rrf_merge
    ...
    def reindex(self) -> Dict[str, Any]:
        ...
        Scans all receipt directories across corpora,
        ...
        for corpus_id, receipt_dir in iter_receipt_dirs():
            chunks_file = receipt_dir / "chunks.jsonl"
            if not chunks_file.exists():
                continue
            ...
            for line in chunks_file.read_text(encoding="utf-8").splitlines():
                ...
                try:
                    ...
                    corpus_ids.append(chunk.get("corpus_id", corpus_id))
        ...
        bm25.build(texts, ids, receipt_ids, corpus_ids)
        bm25.save()
        ...
        try:
            ...
            if DenseIndex.is_available():
                dense = DenseIndex()
                dense.build(texts, ids, receipt_ids, corpus_ids)
                dense.save()
