...
Two-phase install with prerequisite checking before any pip work:
  ...
  PREFLIGHT → WAITING_ON_USER → INSTALLING → STARTING_SERVICES → DONE

Phase 1 (Preflight):  Check Python, Git, Ollama, Windows Biometric
                      Service, and repo structure BEFORE creating .venv.
                      Missing prerequisites → interactive recheck loop
                      with [R]echeck / [O]pen help / [B]oot hook / [Q]uit.
...
GPU support:
  Default install path is CPU-only (no GPU probing at install time).
  Enable GPU explicitly with:
    --enable-gpu --gpu auto   Detect AMD (ROCm) or NVIDIA (CUDA)
    --enable-gpu --gpu rocm   Force AMD ROCm 7.1 torch wheel
    --enable-gpu --gpu cuda   Force NVIDIA CUDA 12.4 torch wheel

Usage:
    python scripts/clearbox_install.py
    python scripts/clearbox_install.py --enable-gpu --gpu auto
    python scripts/clearbox_install.py --enable-gpu --gpu cuda
...
CRITICAL_DIRS = ["security", "bridges", "scripts", "ui", "routing",
                 "plugins/lakespeak", "plugins/wolf_engine"]

# Data directories under the selected data root (see --data-root / picker).
# Must match security/data_paths.py and gateway write zones.
DATA_SUBDIRS = [
    ...
    # Retrieval engines
    "data/indexes/lakespeak/index/chunks",
    ...
    "data/indexes/wolf/index",
    "data/indexes/wolf/events",
    "data/indexes/wolf/eval",
    # Logs + governed write zones
    "data/logs/observe",
    "data/logs/audit",
...
# Torch index URLs per GPU backend
_TORCH_INDEX = {
    "rocm": "https://download.pytorch.org/whl/rocm7.1",
    "cuda": "https://download.pytorch.org/whl/cu124",
}

# llama_cpp_python pre-built wheel indexes (avoids MSVC / nmake requirement)
# Source: https://github.com/abetlen/llama-cpp-python/releases
...
def _build_requirements(include_gpu_checks: bool = False) -> List[Requirement]:
    """Build the prerequisite manifest. GPU checks are opt-in."""
    reqs: List[Requirement] = [
        ...
        Requirement(
            id="msvc_cmake", label="MSVC Build Tools / CMake", type="tool",
            detect=_detect_msvc_or_cmake,
            help_url="https://visualstudio.microsoft.com/visual-cpp-build-tools/",
            severity="recommended",
        ),
        Requirement(
            id="ollama", label="Ollama", type="service",
            detect=_detect_ollama,
            help_url="https://ollama.com/download",
            reboot_hint=True,
        ),
        Requirement(
            id="wbiosrvc", label="Windows Biometric Service", type="service",
...
def _configure_data_root(requested_path: Optional[str], *, interactive: bool) -> bool:
    ...
    default_root = Path("D:/CLEARBOX")
    if not interactive:
        for candidate in (default_root, Path.home() / "ClearboxAI"):
            if _is_writable_dir(candidate):
                _set_data_root(candidate, persist=False)
                _warn(f"Falling back to default data root: {candidate}")
                return True
        _err("No writable default data root found.")
        return False
...
def step_deps(gpu_type: str = "cpu") -> bool:
    ...
    # ── Torch: install with the correct GPU wheel first ───────────────────────
    _info(f"GPU backend selected: {gpu_type.upper()}")
    torch_cmd = [str(venv_py), "-m", "pip", "install", "torch>=2.4.0"]
    if gpu_type in _TORCH_INDEX:
        index_url = _TORCH_INDEX[gpu_type]
        torch_cmd += ["--index-url", index_url]
        _ok(f"PyTorch ({gpu_type.upper()}) — {index_url}")
    else:
        _ok("PyTorch (CPU-only build)")

    r = _run(torch_cmd, timeout=600, stream=True)
    if r.returncode != 0:
        _warn(f"Torch pre-install warning (will retry via requirements): {r.stdout[:300]}")
        # Non-fatal — requirements.txt still has a commented fallback reference

    # ── llama_cpp_python: pre-built wheel to avoid MSVC / nmake ──────────────
    # The abetlen wheel index uses a +cu124 local version tag, so pinning to
    # ==X.Y.Z causes pip to miss the binary and fall back to source.
    # Solution: install without a version pin (--no-deps; lock file covers deps),
    # then exclude llama-cpp-python from the lock file install below.
    llama_index = _LLAMA_CPP_WHEEL_INDEX.get(gpu_type)
    llama_installed = False
    if llama_index:
        _info(f"Pre-installing llama-cpp-python from pre-built wheel ({gpu_type.upper()})...")
        r = _run(
        ...
        if r.returncode == 0:
            _ok("llama-cpp-python installed (pre-built, no compiler needed)")
            llama_installed = True
        else:
            _warn("Pre-built wheel unavailable — source build will be attempted")
            _info("Source build requires Visual Studio Build Tools (MSVC):")
            _info("  https://visualstudio.microsoft.com/visual-cpp-build-tools/")
...
def print_manifest() -> None:
    ...
    categories = [
        ...
        ("plugins/lakespeak/",        "plugins/lakespeak",        (".py",)),
        ("plugins/wolf_engine/",      "plugins/wolf_engine",      (".py",)),
        ("plugins/reasoning_engine/", "plugins/reasoning_engine", (".py",)),
        ("plugins/chat_packs/",       "plugins/chat_packs",       (".py",)),
        ("plugins/clearbox_node/",      "plugins/clearbox_node",      (".py",)),
        ("plugins/help_system/",      "plugins/help_system",      (".py",)),
...
def run_state_machine(args: argparse.Namespace) -> int:
    ...
    # ── GPU mode resolution (CPU-first; GPU opt-in only) ────────
    requested_gpu = args.gpu
    wants_gpu = args.enable_gpu or requested_gpu != "cpu"
    if not args.resume:
    ...
    if state.gpu_enabled:
        ...
        elif requested_gpu == "auto":
            _info("GPU mode enabled — detecting backend...")
            state.gpu_type = _detect_gpu_type()
        elif state.gpu_type == "cpu":
            _info("GPU mode enabled — detecting backend...")
            state.gpu_type = _detect_gpu_type()

        if state.gpu_type == "rocm":
            _ok("GPU backend: ROCm (AMD)")
        elif state.gpu_type == "cuda":
            _ok("GPU backend: CUDA (NVIDIA)")
        else:
            _warn("GPU requested but not detected — continuing in CPU mode")
            state.gpu_type = "cpu"
            state.gpu_enabled = False
    else:
        state.gpu_type = "cpu"
        _ok("GPU mode: disabled (CPU-first install)")
        _info("Enable explicitly with: --enable-gpu --gpu auto")
    ...
    while state.state != InstallState.DONE:
        ...
        if state.state == InstallState.PREFLIGHT:
            ...
            if failures:
                state.state = InstallState.WAITING_ON_USER
                state.save()
                # fall through
            else:
                if warnings:
        ...
        if state.state == InstallState.WAITING_ON_USER:
            requirements = _build_requirements(include_gpu_checks=state.gpu_enabled)
            user_ok = wait_for_user_recheck_loop(requirements, state)
            if not user_ok:
                _info("Exiting. Re-run to continue.")
                state.save()
                return 1
            for req in requirements:
                if req.passed and req.id not in state.passed_checks:
    ...
    _info("  Copy plugins/clearbox_node/ → remote machine")
    _info("  pip install -r requirements.txt")
    _info("  python -m clearbox_node")
...
def main() -> int:
    ...
    ap.add_argument("--gpu", default="cpu",
                    ...
                    help="PyTorch backend (default: cpu; use --enable-gpu for GPU)")
    ap.add_argument("--enable-gpu", action="store_true",
                    help="Enable GPU flow (off by default)")
    ...
    print("    2. Data directories + encrypted config")
    print("    3. NLTK tokenizer data")
    print("    4. Model file verification")
    print("    5. Platform-specific setup (GPU, Ollama)")
