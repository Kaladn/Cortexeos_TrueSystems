...
_PLUGIN_GATEWAY = PluginGatewayService()
...
async def _search_archive(args: dict, session_id: str | None) -> str:
    ...
    try:
        engine = _PLUGIN_GATEWAY.get_lakespeak_engine()
        ...
        return "Archive search unavailable: LakeSpeak plugin not installed."
...
async def _analyze_text(args: dict, session_id: str | None) -> str:
    ...
    try:
        engine = _PLUGIN_GATEWAY.get_wolf_engine()
        result = await asyncio.to_thread(
        ...
        return "Analysis unavailable: Wolf Engine plugin not installed."
    except Exception as e:
