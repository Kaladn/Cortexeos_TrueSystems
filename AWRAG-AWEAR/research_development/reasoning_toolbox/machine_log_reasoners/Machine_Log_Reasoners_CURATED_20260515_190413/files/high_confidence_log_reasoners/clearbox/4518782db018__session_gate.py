"""Clearbox Node -- Windows Hello session gate (FastAPI dependency).

Validates ``clearbox_session`` cookie against the LLM server's auth endpoint.
...
async def require_hello_session(request: Request) -> str:
    """Validate the clearbox_session cookie via the LLM server.
    ...
    """
    from clearbox_node.config import REQUIRE_HELLO

    token = request.cookies.get("clearbox_session", "")
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=AUTH_TIMEOUT) as client:
            resp = await client.get(
                AUTH_STATUS_URL,
                cookies={"clearbox_session": token},
            )
