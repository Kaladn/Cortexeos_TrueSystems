"""Clearbox AI — Tool Registry & Mediated Execution.
...
Safety invariants:
  ...
  - No shell/OS commands — system visibility is read-only via core observe module
...
LOGGER = logging.getLogger("clearbox.tools")

from bridges.services.lakespeak_service import LakeSpeakService


_LAKESPEAK = LakeSpeakService()
...
async def _get_system_metrics(args: dict, session_id: str | None) -> str:
    ...
    from core.observe.reader import _read_cpu, _read_ram, _read_gpu
    import psutil
...
async def _search_archive(args: dict, session_id: str | None) -> str:
    ...
    try:
        ...
        return "Archive search unavailable: LakeSpeak is unavailable."
...
# Shell chaining / redirection patterns that indicate command injection.
# These are blocked in custom tool runner.command templates.
_BLOCKED_CMD_RE = re.compile(
    r"[|;&]"           # pipe, semicolon, ampersand (chaining)
...
def _validate_tool_json(data: dict) -> str | None:
    ...
    # Block shell chaining / redirection in subprocess runner commands
    if rtype in ("powershell", "python", "executable"):
        if _BLOCKED_CMD_RE.search(cmd):
            return (f"Blocked: runner.command contains shell chaining or redirection "
                    f"characters (|, ;, &, >, <, $(), backticks). "
                    f"Use a single command without piping or chaining.")
    return None
...
async def _run_diagnostic(args: dict, session_id: str | None) -> str:
    """Run the ClearboxAI diagnostic suite."""
    try:
        import io
...
# Public API — consumed by clearbox_bridge_server.py
...
# Path 5: malformed open tag drift:
#   e.g. "portun {...}</tool_call>" or "<tool_call {...}</tool_call>"
_TOOL_CALL_CLOSE_ONLY_RE = re.compile(
    r"(?:<[^>\n]*>\s*)?[^{}]*?(\{\s*\"name\"\s*:\s*\"[^\"]+\"\s*,\s*\"arguments\"\s*:\s*\{.*?\}\s*\})\s*</tool_call>",
    re.DOTALL,
)
...
_ALL_TOOL_PATTERNS = [
    _TOOL_CALL_RE,
    _TOOL_CALL_FENCE_RE,
    _TOOL_CALL_BACKTICK_RE,
    _TOOL_CALL_BARE_RE,
    _TOOL_CALL_CLOSE_ONLY_RE,
]
...
def parse_tool_calls(text: str) -> list[dict]:
    ...
    # Path 5: malformed opening tag but valid closing tag + JSON payload
    for m in _TOOL_CALL_CLOSE_ONLY_RE.finditer(text):
        call = _try_extract_call(m.group(1), "close_only")
        if call:
            return [call]

    # Path 6: final salvage — if a close tag exists, scan backwards for JSON.
    if "</tool_call>" in text:
        prefix = text.split("</tool_call>", 1)[0]
        brace_positions = [i for i, ch in enumerate(prefix) if ch == "{"]
        for start in reversed(brace_positions[-8:]):
            call = _try_extract_call(prefix[start:].strip(), "close_scan")
            if call:
                return [call]
