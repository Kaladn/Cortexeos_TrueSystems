...
def analyze_report(report_path: Path) -> Tuple[MappingStats, Dict[str, UnmappedToken]]:
    ...
    if not isinstance(items, dict):
        LOGGER.warning(f"Report {report_path} has no items dict")
        return stats, unmapped
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--reports-root",
        type=Path,
        default=Path("forest_ai/data/maps"),
...
def discover_reports(root: Path) -> List[Path]:
    """Find all 616_map.json files under root."""
    if not root.exists():
        return []
    return sorted(root.glob("**/616_map.json"))
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    output_path = args.output or Path("forest_ai/data/unmapped_tokens_review.json")
