"""
Gaming Domain: Hit Registration Operator V2 (Patent-Enhanced)

Core Loop Stage: APPLY (Detector type)

ENHANCEMENTS OVER V1:
  - Patent signature matching (US20190091581A1)
  - Skill-based tolerance detection
  - Forensic evidence citations
  - Per-weapon baseline tracking

Purpose:
  Detects damage output manipulation by tracking hit marker ghosting.
  NOW WITH: Patent-documented manipulation detection.
  ...
  PATENT DETECTION:
    - US20190091581A1 [0083]: Hit tolerance modification by skill level
    - Real-time accuracy requirement manipulation
...
Outputs:
  - OperatorResult with:
    ...
    - damage_confirm_count: Number of damage confirmations
...
class HitRegistrationOperatorV2:
    ...
    def __init__(self, config_path: str, signature_path: Optional[str] = None):
        ...
        self.hit_marker_region_size = op_config.get("hit_marker_region_size", 0.15)
        ...
        # Ghosting thresholds
        self.ghost_threshold = op_config.get("ghost_threshold", 0.3)
        ...
        print(f"[+] HitRegistrationOperatorV2 initialized (PATENT-ENHANCED)")
    ...
    def analyze(self, seq: FrameSequence) -> Optional[OperatorResult]:
        """
        APPLY stage: Detect hit registration anomalies WITH PATENT MATCHING.
        
        Returns:
          OperatorResult with hit registration metrics, manipulation flags,
        ...
        patent_match = self.patent_matcher.match_signature("hit_registration", detection_data)
