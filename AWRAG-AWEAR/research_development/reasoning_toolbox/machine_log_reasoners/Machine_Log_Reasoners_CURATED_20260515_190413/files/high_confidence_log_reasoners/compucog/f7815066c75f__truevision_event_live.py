...
class CognitiveHarness:
    ...
    Responsibilities:
    1. Capture TrueVision windows (32x32 grids, operators, EOMM)
    ...
    def __init__(
        ...
        self.session_id = session_id or f"session_{int(self.chronos.now())}"
        ...
        if self.enable_events:
            ...
            # Activity logger integration (optional)
            # Use helper to keep logic testable and isolated
        ...
        self.start_time = self.chronos.now()
    ...
    def run(self, duration: int = 30, verbose: bool = True) -> None:
        ...
        last_stats_time = self.chronos.now()
        stats_interval = 5.0  # Print stats every 5 seconds
        
        start = self.chronos.now()
        window_counter = 0
        
        try:
            while self.chronos.now() - start < duration:
                # Capture frame
                ...
                # Convert to grid
                frame_timestamp = self.chronos.now()
                ...
                if verbose and self.chronos.now() - last_stats_time >= stats_interval:
    ...
    def _print_progress(self) -> None:
        ...
        elapsed = self.chronos.now() - self.start_time
