...
# ======================================================================
#  Role-mapping utility for lattice_role_recolor
# ======================================================================

def is_partial_role_mapping(inp_roles: Dict, out_roles: Dict) -> bool:
    """
    Allow output to merge roles or drop roles, but not to map
    a single input role to multiple conflicting outputs.
    inp_roles/out_roles: role_id -> set of positions or list of positions
    """
    for role, positions in inp_roles.items():
        # What output-role this input-role "lands in"
        out_role_ids = {rid for rid, rpos in out_roles.items()
                        if any(p in (rpos if isinstance(rpos, (set, list)) else [rpos]) 
                               for p in (positions if isinstance(positions, (set, list)) else [positions]))}
        if len(out_role_ids) > 1:
            # One input role maps into several unrelated output roles → ambiguous
            return False
    return True
...
class SandwichTilingOperator(ArcOperator):
    ...
    def analyze(self, input_grid: np.ndarray,
        ...
        # GENERALIZED: Try multiple tiling factors (2x2, 3x3, 4x4, 5x5)
        for N in [2, 3, 4, 5]:
            if H_out != N * H_in or W_out != N * W_in:
                continue

            # Try different rotation patterns
            base = input_grid
            
            # Pattern 1: Alternating base/rotated (original 3x3 sandwich)
            # Row 0, 2, 4...: base, Row 1, 3, 5...: rotated
            for rotation in [1, 2, 3]:  # 90°, 180°, 270° clockwise
                rot = np.rot90(base, -rotation)
                
                try:
                    rows = []
                    for row_idx in range(N):
                        if row_idx % 2 == 0:
                            row = np.hstack([base] * N)
                        else:
                            row = np.hstack([rot] * N)
                        rows.append(row)
                ...
                if np.array_equal(expected, output_grid):
                    return {
                        "pattern": "alternating",
                        "grid_size": (N, N),
            ...
            # Pattern 2: Checkerboard (alternating within row too)
    ...
    def apply(self, input_grid: np.ndarray,
              params: Dict[str, Any]) -> np.ndarray:
        base = input_grid
        N, _ = params["grid_size"]
        rotation_deg = params["rotation"]
        pattern = params["pattern"]
        
        # Get rotated version
        rot = np.rot90(base, -(rotation_deg // 90))
        
        if pattern == "alternating":
            # Alternating rows: base, rot, base, rot, ...
            rows = []
            for row_idx in range(N):
                if row_idx % 2 == 0:
                    row = np.hstack([base] * N)
...
class LargestBlobExtractOperator(ArcOperator):
    ...
    def analyze(self, input_grid: np.ndarray,
        ...
        # RELAXED: Allow minor color variations, focus on shape extraction
        # output should not introduce colors not in input
        input_colors = set(int(x) for x in np.unique(input_grid))
        output_colors = set(int(x) for x in np.unique(output_grid))
        
        if not (output_colors <= input_colors):
            return None  # Output has colors not in input
...
class LatticeRoleRecolorOperator(ArcOperator):
    ...
    def apply(self, input_grid: np.ndarray,
        ...
        # Step 4: Apply the mapping using lattice position as primary key
        out = np.zeros_like(input_grid)
        for r in range(H):
            for c in range(W):
                lattice_pos = (r % k, c % m)
                
                # Primary: Use output pattern for this lattice position
                # This handles the common case where pattern is position-determined
...
class LatinSquareFromDiagonalOperator(ArcOperator):
    ...
    def analyze(self, input_grid: np.ndarray,
        ...
        # GENERALIZED: Extract NxN pattern and verify Latin square structure
        # Try N=2,3,4,5
        for N in [2, 3, 4, 5]:
            if H_out < N or W_out < N:
                continue
            
            # Extract NxN pattern from output
            pattern_NxN = output_grid[:N, :N]
            
            # Verify it tiles correctly
            tiles_ok = True
            for r in range(H_out):
                for c in range(W_out):
                    expected = pattern_NxN[r % N, c % N]
            ...
            # Latin square rotation: each row is a cyclic shift of first row
            first_row = [int(pattern_NxN[0, col]) for col in range(N)]
            
            latin_square_ok = True
            for row_idx in range(N):
                for col_idx in range(N):
                    expected_val = first_row[(col_idx - row_idx) % N]
                    actual_val = int(pattern_NxN[row_idx, col_idx])
                    if actual_val != expected_val:
                        latin_square_ok = False
                        break
                if not latin_square_ok:
                    break
            
            if not latin_square_ok:
                continue
            
            # Found valid Latin square with period N
            # Store for later verification
            found_N = N
            found_colors = first_row
            break
        else:
            # No valid Latin square found
            return None
        ...
        # GENERALIZED: Support N colors for NxN Latin squares (N=2,3,4,5)
        N = len(unique_input_colors)
        if N < 2 or N > 5:
            return None
        
        # Check if output uses NxN pattern
        if H_out < N or W_out < N:
            return None
        
        # Output colors must match input colors (no new colors)
        output_colors = set(found_colors)
        if len(unique_input_colors) != len(found_colors):
            return None
        if set(unique_input_colors) != output_colors:
            return None
        
        # Success: found NxN Latin square pattern
        return {
            "N": found_N,
            "colors": found_colors,
...
class GrowShrinkOperator(ArcOperator):
    ...
    def analyze(self, input_grid: np.ndarray,
        ...
        # RELAXED: Allow multiple colors, but input and output must have same colors
        if not np.array_equal(sorted(in_colors), sorted(out_colors)):
            return None

        # Try single-color case first (original behavior)
        if len(in_colors) == 1:
            fg = int(in_colors[0])
            ...
            if np.array_equal(dil, out_mask) and not np.array_equal(inp_mask, out_mask):
                return {"mode": "dilate", "colors": [fg]}

            # Try erosion check
            ero = self._erode(inp_mask)
            if np.array_equal(ero, out_mask) and not np.array_equal(inp_mask, out_mask):
                return {"mode": "erode", "colors": [fg]}
        
        # NEW: Try all-colors morphology (apply same operation to all non-zero)
        inp_mask = (input_grid != 0)
        out_mask = (output_grid != 0)
        
        # Check if dilate works on combined mask
        dil = self._dilate(inp_mask)
        if np.array_equal(dil, out_mask) and not np.array_equal(inp_mask, out_mask):
            # Verify colors are preserved
            if self._verify_color_preservation(input_grid, output_grid, inp_mask, dil):
                return {"mode": "dilate", "colors": "all"}
        
        # Check if erode works on combined mask
        ero = self._erode(inp_mask)
        if np.array_equal(ero, out_mask) and not np.array_equal(inp_mask, out_mask):
            # Verify colors are preserved
            if self._verify_color_preservation(input_grid, output_grid, inp_mask, ero):
                return {"mode": "erode", "colors": "all"}

        return None

    def _verify_color_preservation(self, input_grid: np.ndarray, output_grid: np.ndarray,
                                     old_mask: np.ndarray, new_mask: np.ndarray) -> bool:
        """Verify that morphology preserves colors where cells remain."""
        H, W = input_grid.shape
        for r in range(H):
            for c in range(W):
                if old_mask[r, c] and new_mask[r, c]:
                    # Cell was present before and after - color must match
    ...
    def apply(self, input_grid: np.ndarray,
        ...
        if colors is None and "fg" in params:
            colors = [int(params["fg"])]
        
        if colors == "all":
            # Apply morphology to all non-zero pixels, preserving colors
            mask = (input_grid != 0)
            ...
            # For dilation: new cells get color from nearest neighbor
            # For erosion: just remove cells
            if mode == "erode":
                out = np.copy(input_grid)
                out[~new_mask] = 0
            else:
                # Dilate: propagate colors to new cells
                out = np.copy(input_grid)
                H, W = input_grid.shape
                for r in range(H):
                    for c in range(W):
                        if new_mask[r, c] and not mask[r, c]:
                            # New cell - find color from neighbors
                            for nr, nc in self._neighbors4(H, W, r, c):
                                if mask[nr, nc]:
                                    out[r, c] = input_grid[nr, nc]
                                    break
            return out
        else:
            ...
            if mode == "dilate":
                out_mask = self._dilate(mask)
            else:  # erode
                out_mask = self._erode(mask)
...
# OPERATOR REGISTRY - Phase 2 (2025-11-30)
# 18 working operators (56 solves) + 11 being fixed (gateway to ~100-120 solves)
OPERATORS: List[ArcOperator] = [
    # ===== WORKING OPERATORS (18) =====
    # Simple / primitive recolor/replicate
    SwapMappingOperator(),             # 1 solve
    RecolorMappingOperator(),          # 10 solves
    ColorHistogramFillOperator(),      # 1 solve
    HorizontalReplicateOperator(),     # 1 solve

    # Positional & block-wise patterns
    PositionBasedRecolorOperator(),    # 20 solves - WORKHORSE
    GridSubdivisionOperator(),         # 1 solve

    # Structural / geometric
    MirrorOperator(),                  # 2 solves
    CropToBoundingBoxOperator(),       # 1 solve
    
    # Spatial reasoning
    CenterRepulsionOperator(),         # 1 solve
    
    # Center-anchor patterns
    DiagonalReflectionOperator(),      # 1 solve
    CenterHaloExpansionOperator(),     # 1 solve
    HollowFrameExtractionOperator(),   # 1 solve
    BlockExpansionOperator(),          # 6 solves
    SymmetryCompletionOperator(),      # 2 solves

    # Tiling families
    TilingExpandOperator(),            # 2 solves
    SelfMaskingTilingOperator(),       # 2 solves
    
    # ===== BEING FIXED (11) - Gateway to 944 unsolved =====
    # These cover missing pattern families:
    # - Object Morphology (grow_shrink)
    # - Latin Square patterns (latin_square_from_diagonal)
    # - Lattice role mapping (lattice_role_recolor)
    # - Structured recoloring (structured_position_recolor)
    # - Blob extraction (largest_blob_extract)
    # - Advanced tiling (self_referential_tiling, sandwich_tiling)
    # - Diagonal markers (diagonal_marker_toroidal)
    
    GrowShrinkOperator(),              # FIXED: multi-color morphology
    SandwichTilingOperator(),          # FIXED: generalized NxN
    LatticeRoleRecolorOperator(),      # TO FIX: merge logic
    StructuredRecolorOperator(),       # TO FIX: merge logic
    LatinSquareFromDiagonalOperator(), # TO FIX: N-color generalization
    LargestBlobExtractOperator(),      # TO FIX: validation constraints
    SelfReferentialTilingOperator(),   # TO FIX: may be redundant with TilingExpand
    DiagonalMarkerOperator(),          # TO FIX: relax toroidal assumption
]
...
if operators_dir.exists():
    # Import working operators + being-fixed operators
    try:
        ...
        from repeat_operator import RepeatOperator
        
        # Add to operators list
        OPERATORS.extend([
            EnclosedRegionFillOperator(),           # 1 solve
            ShapeReferenceRecolorOperator(),        # 2 solves
            CheckerboardTilingAlternatingOperator(),  # TO FIX
            SymmetryFromFragmentOperator(),          # TO FIX
            PatternCompletionPrototypeOperator(),    # TO FIX
            RepeatOperator(),                        # NEW - tiling/repetition (30+ tasks)
        ])
        
        print(f"✓ Loaded 6 operators from {operators_dir}")
    except Exception as e:
        print(f"Warning: Could not load operators: {e}")
else:
...
def merge_operator_params(op: ArcOperator,
    ...
    base = params_list[0]
    for p in params_list[1:]:
        # Handle dict comparison including numpy arrays
        try:
            if p != base:
                return None
        except ValueError:
            # Numpy array comparison - need element-wise check
            if set(p.keys()) != set(base.keys()):
                return None
            for key in base.keys():
                val_base = base[key]
                val_p = p[key]
                if isinstance(val_base, np.ndarray) and isinstance(val_p, np.ndarray):
                    if not np.array_equal(val_base, val_p):
                        return None
                elif isinstance(val_base, np.ndarray) or isinstance(val_p, np.ndarray):
                    return None
                else:
                    if val_base != val_p:
                        return None
