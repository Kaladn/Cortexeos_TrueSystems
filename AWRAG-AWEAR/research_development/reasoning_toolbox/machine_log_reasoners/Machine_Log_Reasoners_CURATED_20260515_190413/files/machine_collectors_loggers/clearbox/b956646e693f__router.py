"""Clearbox Network — main API router.

Mounted at /api/network/ on the Clearbox AI Bridge (port 5050).
...
Endpoints (serve — for peer nodes, require X-Clearbox-Node-Key):
...
async def network_health() -> dict[str, Any]:
    """Core network health check. Returns node count from config."""
    nodes = get_nodes()
    return {
        "ok": True,
        "service": "clearbox_network",
        "phase": 1,
...
async def network_read(node_id: str, path: str) -> StreamingResponse:
    ...
    if _is_local(node_id):
        ...
        return StreamingResponse(
            ...
            headers={"Content-Length": str(file_size), "X-Clearbox-Node": node_id},
        )
