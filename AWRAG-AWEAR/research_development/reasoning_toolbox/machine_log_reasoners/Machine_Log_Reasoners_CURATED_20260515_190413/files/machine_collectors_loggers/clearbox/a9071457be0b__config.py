"""Clearbox Network — configuration loader.

Reads clearbox.config.json and extracts:
...
LOGGER = logging.getLogger("clearbox.network.config")

# Repo root: plugins/clearbox_network/config.py → ../../
_REPO_ROOT = Path(__file__).resolve().parent.parent.parent
_CONFIG_FILE = _REPO_ROOT / "clearbox.config.json"
...
def _load_raw() -> dict[str, Any]:
    """Load and parse clearbox.config.json. Returns {} on any error."""
    try:
        return json.loads(_CONFIG_FILE.read_text(encoding="utf-8"))
    except FileNotFoundError:
        LOGGER.warning("clearbox.config.json not found at %s", _CONFIG_FILE)
        return {}
    except json.JSONDecodeError as exc:
        LOGGER.error("clearbox.config.json parse error: %s", exc)
        return {}


def get_network_config() -> dict[str, Any]:
    """Return the 'network' section of clearbox.config.json (or empty dict)."""
...
def is_auth_required() -> bool:
    """Return True if X-Clearbox-Node-Key auth is required on serve endpoints."""
    return bool(get_network_config().get("require_auth", True))


def get_nodes() -> list[dict[str, Any]]:
    """Return the full nodes list from clearbox.config.json."""
...
def get_allow_root_paths() -> list[str]:
    ...
    E.g. ["docs/GENESIS", "Lexical Data/Canonical", "clearbox_ai/data"]
    """
    roots = get_network_config().get("allow_roots", [])
    result: list[str] = []
    for r in roots:
...
def get_peer_keys() -> list[str]:
    ...
    These are the keys that remote peers present in X-Clearbox-Node-Key.
