"""GPU execution utilities for Clearbox AI bridges.
...
# Native C++ engine — try to import for CPU-accelerated window counts + anchor extraction
try:
    import clearbox_engine as _native  # type: ignore
...
if _NATIVE_AVAILABLE:
    LOGGER.info("🔧 Native C++ engine loaded (clearbox_engine) — CPU-accelerated window counts + anchor extraction")
else:
    LOGGER.info("⚠️ Native C++ engine not found — using Python/PyTorch fallback")
...
def compute_window_counts(
    ...
    "after") to a list of tuples ``(focus_id, context_id, count)``.

    Priority: C++ native > GPU (PyTorch) > Python fallback.
    """
    ...
    # ── Native C++ path (fastest on CPU — packed hash map, no GIL) ──────
    if _NATIVE_AVAILABLE and not device.is_cuda:
        LOGGER.debug("compute_window_counts: native C++ path (%d tokens)", len(token_ids))
        return _native.compute_window_counts(token_ids, window)

    # ── GPU path: vectorized operations in PyTorch ──────────────────────
    if torch is not None and device.is_cuda:
    ...
    # ── Python fallback (no native, no GPU) ─────────────────────────────
    LOGGER.debug("compute_window_counts: Python fallback (%d tokens)", len(token_ids))
    counts_map: Dict[int, Dict[Tuple[int, int], int]] = {}
    ...
    for idx, focus in enumerate(token_ids):
        for offset in range(1, window + 1):
            ...
            counts_map.setdefault(-offset, {})[pair] = counts_map.setdefault(-offset, {}).get(pair, 0) + 1
        for offset in range(1, window + 1):
    ...
    return {
        ...
        for offset, pairs in counts_map.items()
    }
