"""Governed I/O — bridges the ClearboxLexiconBridge to gateway-managed storage.
...
# ── Contract B/D: Shared mapped predicate ─────────────────────
# Both governed_io.py and map_manager.py must use this function.
# Mapped ⟺ lexicon_payload is present and non-empty (not None, not {}).
def _is_lexicon_matched(info: dict) -> bool:
    """Return True if the token has a non-empty lexicon_payload."""
    payload = info.get("lexicon_payload")
...
def write_map_report(
    ...
    mapped_count = sum(
        ...
        if isinstance(info, dict) and _is_lexicon_matched(info)
    )
