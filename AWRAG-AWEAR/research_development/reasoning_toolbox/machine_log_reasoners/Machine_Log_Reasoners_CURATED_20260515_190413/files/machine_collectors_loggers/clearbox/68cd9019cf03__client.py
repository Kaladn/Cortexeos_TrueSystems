"""Clearbox Network — outbound client for bridge-to-bridge requests.

Makes authenticated HTTP requests from this node to remote peer nodes.
All requests carry X-Clearbox-Node-Key using the peer's configured api_key.
...
Raises:
    NodeNotFoundError — node_id not in clearbox.config.json
...
LOGGER = logging.getLogger("clearbox.network.client")
...
class NodeNotFoundError(Exception):
    """node_id was not found in clearbox.config.json nodes list."""
...
def _resolve_node(node_id: str) -> tuple[dict[str, Any], str, str]:
    ...
    if not node:
        raise NodeNotFoundError(f"Node {node_id!r} is not in clearbox.config.json")
    api_key = _key(node)
    if not api_key:
        raise NodeAuthError(
...
async def browse(node_id: str, path: str = "/") -> dict[str, Any]:
    ...
    headers = {"X-Clearbox-Node-Key": api_key}
...
async def read_stream(
    ...
    response_headers keys: Content-Length, X-Clearbox-Path, X-Clearbox-Node.
    """
    node, base, api_key = _resolve_node(node_id)
    ...
    resp_headers: dict[str, str] = {
        ...
        "X-Clearbox-Path": resp.headers.get("x-clearbox-path", path),
        "X-Clearbox-Node": node_id,
    }
