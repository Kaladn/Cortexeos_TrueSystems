...
# Clearbox Network plugin — Phase 1 smoke tests.
#
...
def _plugin_available() -> bool:
    """Return True if clearbox_network plugin is mounted and healthy."""
    try:
...
if not _plugin_available():
    pytest.skip(
        f"clearbox_network plugin not available at {HOST}:{PORT}/api/network — "
...
def test_health_ok():
    """Plugin health endpoint returns ok=True."""
    status, data = _get("/api/network/health")
    assert status == 200, f"Expected 200, got {status}: {data}"
    assert data.get("ok") is True
    assert data.get("plugin") == "clearbox_network"
