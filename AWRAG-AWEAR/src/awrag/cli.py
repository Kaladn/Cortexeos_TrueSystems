...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        description="AWEAR dataset-local evidence engine CLI.",
        epilog="""
Batch walkthrough:
  1. Create a plain text file with one question per line.
  2. Make sure the dataset has already been built with awear intake.
  3. Run: awear batch --runtime-root <runtime> --dataset <name> --questions questions.txt
  4. Watch the tqdm progress bar in the terminal.
  5. Open the reported batch_run_summary.json when complete.
""",
    )
    parser.add_argument("--version", action="version", version="awear 0.05")
    sub = parser.add_subparsers(dest="command", required=True)

    init_cmd = sub.add_parser("init", help="Create a dataset-local AWEAR scope")
...
""",
    ...
    overview_cmd = sub.add_parser(
        ...
        epilog="""
Step-by-step:
  1. Build a dataset with awear intake.
  2. Run:
       awear dataset-overview --runtime-root <runtime> --dataset <name> --out <overview-folder>
...
""",
    ...
    deeper_wider_cmd.add_argument(
        ...
        type=Path,
        required=True,
        help="Query-result JSON or awear_anchor_prediction_walk@1 JSON already returned by the first pass.",
    )
    ...
    packet_speech_cmd = sub.add_parser(
        ...
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description="Report-only speech from existing AWEAR query packet JSON. Does not run retrieval, topK, intake, or model reasoning.",
        epilog="""
Step-by-step:
  1. Run awear query and keep the reported output_path.
  2. Run:
       awear packet-speech --packet <query-output.json> --out <speech-output-folder>
...
""",
    ...
    packet_speech_cmd.add_argument("--packet", action="append", type=Path, required=True, help="Existing AWEAR query JSON. Repeat for multiple packets.")
...
Step-by-step:
  1. Run awear query and keep the reported output_path.
  2. Run:
       awear evidence-speech --packet <query-output.json> --out <speech-output-folder>
...
""",
    ...
    evidence_speech_cmd.add_argument("--packet", type=Path, required=True, help="Existing AWEAR query result JSON.")
    ...
    pressure_probe_cmd = sub.add_parser(
        ...
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description="Off-path thinking diagnostic. Reads existing AWEAR query packets and writes second-layer evidence questions. Does not run retrieval, topK, intake, speech, models, or dataset mutation.",
        epilog="""
Step-by-step:
  1. Run awear query or awear batch and keep the query JSON or batch_run_summary.json.
  2. Run:
       awear pressure-probe --packet <query-output.json> --out <probe-folder>
     or:
       awear pressure-probe --batch-summary <batch_run_summary.json> --out <probe-folder>
  3. Open questions/PRESSURE_PROBE_QUESTIONS.jsonl.
  4. If the probes earn it, feed that JSONL back through normal awear batch.
  5. Receipts prove no retrieval, ranking, intake, speech, answer generation, or dataset mutation ran in this sidecar.
""",
    )
    pressure_probe_cmd.add_argument("--packet", action="append", type=Path, help="Existing AWEAR query result JSON. Repeat for multiple packets.")
    pressure_probe_cmd.add_argument("--batch-summary", type=Path, help="Existing awear batch_run_summary.json to probe all completed packets.")
    ...
    topk_diag_cmd = sub.add_parser(
        ...
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description="Off-path retrieval diagnostic. Reads existing AWEAR query packets or batch summaries and walks TopK by rank layer. Does not run retrieval, intake, ranking mutation, or dataset mutation.",
        epilog="""
Step-by-step:
  1. Run awear query or awear batch and keep the query JSON or batch_run_summary.json.
  2. Run:
...
""",
    ...
    topk_diag_cmd.add_argument("--packet", action="append", type=Path, help="Existing AWEAR query result JSON. Repeat for multiple packets.")
    topk_diag_cmd.add_argument("--batch-summary", type=Path, help="Existing awear batch_run_summary.json.")
...
Step-by-step:
  ...
  2. Run:
       awear answer-reasoning-reverse-walk --runtime-root <runtime> --dataset <name> --claims claims.jsonl --out <folder>
...
""",
    ...
    pressure_coordination_cmd = sub.add_parser(
        ...
        epilog="""
Step-by-step:
  1. Run awear answer-reasoning-reverse-walk and keep ANSWER_REASONING_REVERSE_WALK_TRACE.jsonl.
  2. Run:
       awear pressure-coordination-audit --trace <trace.jsonl> --out <folder>
...
Step-by-step:
  1. Build a dataset with awear intake.
  2. Run:
       awear count-walk-speech --runtime-root <runtime> --dataset <name> --question "..." --out <folder>
...
""",
    ...
    batch_cmd = sub.add_parser(
        ...
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description="Run many dataset questions through the existing AWEAR query path.",
        epilog="""
Step-by-step:
  1. Put one question per line in questions.txt.
  2. Blank lines are ignored.
  3. Run:
       awear batch --runtime-root <runtime> --dataset <name> --questions questions.txt --workers 4
...
""",
    ...
    codex_cmd = sub.add_parser("stage-codex", help="Stage Codex session JSONL as AWEAR chat-turn markdown")
    ...
    codex_md_cmd = sub.add_parser("stage-codex-md", help="Stage visible Codex Markdown chat export as AWEAR chat-turn markdown")
    codex_md_cmd.add_argument("--input", type=Path, required=True)
    codex_md_cmd.add_argument("--output", type=Path, required=True)

    chatgpt_cmd = sub.add_parser("stage-chatgpt", help="Stage ChatGPT data export conversations as AWEAR chat-turn markdown")
...
Step-by-step:
  ...
  2. Make sure the dataset has already been built with awear intake.
  3. Run:
       awear special-search --runtime-root <runtime> --dataset-id <dataset> --trigger-list triggers.json --out reports/special_search
...
def _format_laptop_temp_intake_summary(result: dict[str, object]) -> str:
    ...
    lines = [
        "",
        "AWEAR laptop-temp-intake complete",
