...
def build_topk_pressure_trellis(
    ...
    *,
    ...
    forward viability.
    """
    q = {str(anchor).casefold() for anchor in (question_anchors or []) if str(anchor).strip()}
    gaps = {str(gap).casefold() for gap in (open_gaps or []) if str(gap).strip()}
    ...
    return {
        "schema": "awear_topk_pressure_trellis@1",
...
def _candidate_anchor(candidate: dict[str, Any]) -> str:
    return str(candidate.get("anchor") or candidate.get("symbol") or candidate.get("text") or "").casefold()
...
def _rear_coherence(
    ...
    for distance, previous in enumerate(reversed(history[-6:]), start=1):
        previous_anchor = str(previous.get("anchor") or "").casefold()
...
def _question_pressure(candidate: dict[str, Any], anchor: str, question_anchors: set[str]) -> float:
    ...
    if anchor in question_anchors:
        return 1.0
    related = {str(value).casefold() for value in candidate.get("related_question_anchors") or []}
...
def _gap_closure(candidate: dict[str, Any], anchor: str, open_gaps: set[str]) -> tuple[float, list[str]]:
    if not open_gaps:
        return 0.0, []
    closes = {str(value).casefold() for value in candidate.get("closes_gaps") or []}
...
def _redundancy_penalty(anchor: str, history: list[dict[str, Any]]) -> float:
    return min(1.0, sum(1 for row in history[-6:] if str(row.get("anchor") or "").casefold() == anchor) * 0.35)
