...
from .anchors import STOP_ANCHORS, anchorize, symbol_hex
...
def run_answer_reasoning_reverse_walk(
    *,
    ...
    summary = with_protected_notice({
        "schema": "awrag_answer_reasoning_reverse_walk_run@0",
        "created_at": utc_now(),
...
def _meaningful_anchors(text: str) -> list[str]:
    ...
    for anchor in anchorize(text):
        if len(anchor) < 3 or anchor in WEAK_ANCHORS:
            continue
