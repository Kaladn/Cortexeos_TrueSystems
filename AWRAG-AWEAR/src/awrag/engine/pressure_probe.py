...
SCHEMA = "awrag_pressure_probe_run@0"
QUESTION_SCHEMA = "awrag_pressure_probe_question@0"
RECEIPT_SCHEMA = "awrag_pressure_probe_run_receipt@0"
AWCIT_RE = re.compile(r"\[?AWCIT-[0-9A-Za-z]+\]?", re.IGNORECASE)
...
def run_pressure_probe(
    *,
    ...
    This is a sidecar lane. It reads admitted AWEAR evidence packets and writes
    ...
    run_receipt = {
        ...
        "created_at": utc_now(),
        "source": "existing_awrag_query_packets",
...
def build_pressure_probe_record(
    *,
    ...
    for location_index, location in enumerate(locations, start=1):
        for template in _probe_templates_for_location(location):
            ...
            probe_rows.append({
                ...
                "bounded_depth": 1,
                "run_back_through": "awear batch/query only; this sidecar does not run retrieval",
                "boundaries": {
                    "no_answer_generation": True,
                    "no_answer_key_leakage": True,
                    "no_raw_document_guessing": True,
    ...
    return {
        "schema": "awrag_pressure_probe_case@0",
        "case_id": case_id,
...
def clean_pressure_hypothesis_question(question: str) -> str:
    ...
    without_citations = AWCIT_RE.sub(" ", str(question))
    terms = [
        term
...
def _compare_artifact_states(before: list[ArtifactState], after: list[ArtifactState]) -> dict[str, Any]:
    ...
    return {
        "schema": "awrag_pressure_probe_no_mutation_receipt@0",
        "before": before_rows,
        "after": after_rows,
