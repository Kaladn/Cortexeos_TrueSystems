...
from .anchors import STOP_ANCHORS, anchorize, expand_query_anchors, symbol_hex
...
def query(
    ...
    *,
    ...
    anchor_prediction_answer = _anchor_prediction_answer(paths, q_anchors, qualified["locations"], blocks)

    answer_packet = {
        "instruction": "Use cited local evidence coordinates only. This packet is a facsimile output, not source evidence.",
        "citations_owned_by": "AWEAR",
...
def _progress_iter(iterable: Iterable[Any], *, total: int, enabled: bool) -> Iterable[Any]:
    ...
    try:
        ...
        return iterable
    return tqdm(iterable, total=total, desc="AWEAR batch", unit="question")
...
def top_relation_neighbors(paths: DatasetPaths, q_counter: Counter[str], *, limit: int) -> list[dict[str, Any]]:
    ...
    blocked_symbols = {
        symbol
        for anchor in [*q_counter, *STOP_ANCHORS]
...
def _anchor_prediction_answer(
    ...
    if not locations:
        return {
            "schema": "awear_anchor_prediction_answer@1",
    ...
    return {
        ...
        "answer_text": " ".join(prediction["generated_anchors"]),
