...
def _build_intake_resource_plan(
    *,
    ...
    if requested_count < MIN_RUNTIME_WORKERS and not debug_tiny_single_core:
        raise ValueError(f"awear intake requires at least {MIN_RUNTIME_WORKERS} workers; single-core/low-core execution is not allowed")
    ...
    if effective_workers < MIN_RUNTIME_WORKERS and not debug_tiny_single_core:
        raise RuntimeError(
            "awear intake cannot honor the operator compute rule with current CPU/RAM limits; "
    ...
    return {
        ...
        "created_at": utc_now(),
        "preflight_name": "AWEAR RESOURCE PREFLIGHT",
        "resources": resources,
        "source_file_count": len(files),
...
def _progress_iter(iterable: Iterable[Any], *, total: int, enabled: bool) -> Iterable[Any]:
    ...
    try:
        ...
        return iterable
    return tqdm(iterable, total=total, desc="awear intake", unit="file")
