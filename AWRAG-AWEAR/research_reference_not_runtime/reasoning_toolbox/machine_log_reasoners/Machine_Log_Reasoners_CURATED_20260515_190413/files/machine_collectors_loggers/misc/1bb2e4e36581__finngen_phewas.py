...
Fetch FinnGen PheWAS associations for one variant input.
Input JSON on stdin:
  - {"grch38":"10-112998590-C-T"}
  - {"grch37":"10:114758349:C:T","max_results":25}
...
FINNGEN_BASE = "https://r12.finngen.fi"
USER_AGENT = "finngen-phewas-skill/1.0 (+requests)"
...
def parse_input(payload: Any) -> tuple[str, str, int, bool, str | None, float]:
    if isinstance(payload, str):
        return "grch38", payload.strip(), DEFAULT_MAX_RESULTS, False, None, DEFAULT_TIMEOUT_S
...
def fetch_finngen_variant(
    session: requests.Session,
    variant_str: str,
    timeout_sec: float,
) -> tuple[Any | None, int | None]:
    encoded = requests.utils.quote(variant_str, safe=":-")
    url = f"{FINNGEN_BASE}/api/variant/{encoded}"
    headers = {
...
def resolve_raw_output_path(canonical_variant: str, raw_output_path: str | None) -> Path:
    ...
    return Path("/tmp") / f"finngen-phewas-{safe_variant}.json"
...
def main() -> int:
    ...
    session = requests.Session()
    try:
        data, status_code = fetch_finngen_variant(session, parsed["canonical"], timeout_sec)
    except requests.RequestException as exc:
        sys.stdout.write(json.dumps(error("network_error", f"FinnGen request failed: {exc}")))
        return 1
    except ValueError as exc:
        sys.stdout.write(json.dumps(error("invalid_response", f"FinnGen returned non-JSON: {exc}")))
        return 1

    variant_url = f"{FINNGEN_BASE}/variant/{parsed['canonical']}"
    ...
    if status_code == 404:
        warnings.append("Variant not found in FinnGen PheWAS API (HTTP 404).")
        output = {
            "ok": True,
            "source": "finngen",
    ...
    variant_info = data.get("variant") if isinstance(data, dict) else None
    regions = (
        data.get("regions")
