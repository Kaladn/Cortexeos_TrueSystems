...
class CompositionSearch:
    ...
    def find_best_2_step(self, 
        ...
        """
        Search for a valid 2-step operator composition using smart strategy.
        
        KEY INSIGHT: op1.analyze(input, output) gives params for SOME transformation,
        not necessarily input→output. We use whatever op1 produces as intermediate,
        then try op2 to complete the transformation.
        ...
        for op1, op2 in candidates:
            # Check compatibility (DISABLED for testing - try all pairs)
            # if not self.compatible(op1, op2):
            #     continue
            
            # Try op1 on first example to see what transformation it produces
            first_ex = train_pairs[0]
            try:
                inp0 = np.array(first_ex["input"])
                out0 = np.array(first_ex["output"])
                
                # Get params from op1.analyze - this gives SOME transformation
                cfg1 = op1.analyze(inp0, out0)
                if cfg1 is None:
                    continue
                
                # Apply op1 to see what intermediate it creates
                intermediate0 = op1.apply(inp0, cfg1)
                
                # If op1 already produces the output, it's not a composition
            ...
            # Now try op2 on the intermediate → output
            try:
                cfg2 = op2.analyze(intermediate0, out0)
                if cfg2 is None:
                    continue
            except Exception:
                continue
            ...
            for ex in train_pairs:
                try:
                    ...
                    if not np.array_equal(final, expected_out):
                        all_valid = False
                        break
                except Exception:
                    all_valid = False
