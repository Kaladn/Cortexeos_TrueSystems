...
Merges all contract-enriched category files into:
  ...
  - securecore_agents.csv    (filtered safe set for SecureCore)
  - securecore_restricted.csv (blocked/sandbox set)
"""
...
SECURECORE_CATS = {"security", "audit", "observers", "validators", "retrievers", "engines"}
...
def main():
    ...
    # ── Filter into SecureCore vs Restricted ────────────────────
    securecore = []
    restricted = []
    ...
    for r in all_rows:
        ...
        # SecureCore: right category, promoted, safe enough
        if cat in SECURECORE_CATS and promo == "YES" and dscore <= 3:
            securecore.append(r)
        # Everything else stays in full catalog only

    sc_path = os.path.join(OUTPUT_DIR, "securecore_agents.csv")
    with open(sc_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=CATALOG_FIELDS)
        writer.writeheader()
        writer.writerows(securecore)

    rest_path = os.path.join(OUTPUT_DIR, "securecore_restricted.csv")
    ...
    sc_cat_counts = Counter(r["category"] for r in securecore)
    tier_counts = Counter(r["agent_tier"] for r in all_rows)
    score_dist = Counter(int(r["destruction_score"]) for r in all_rows)

    sc_score_dist = Counter(int(r["destruction_score"]) for r in securecore)
    ...
    print(f"SecureCore safe agents:         {safe_count}")
    ...
    print()
    print("SecureCore agents by category:")
    for cat, cnt in sc_cat_counts.most_common():
        print(f"  {cat:25s} {cnt:>5}")

    print()
    print("SecureCore destruction scores:")
    for s in range(6):
