"""Clearbox Network — authentication.

All /api/network/serve/* endpoints require X-Clearbox-Node-Key header.
The key is validated against the peer key list in clearbox.config.json.
...
LOGGER = logging.getLogger("clearbox.network.auth")
_AUDIT = logging.getLogger("clearbox.network.audit")
...
async def require_node_key(
    x_clearbox_node_key: Optional[str] = Header(default=None),
) -> str:
    """FastAPI dependency — validates X-Clearbox-Node-Key header.
    ...
    if not is_auth_required():
        ...
        return x_clearbox_node_key or ""

    if not x_clearbox_node_key:
        _AUDIT.warning("AUTH_REJECT | missing X-Clearbox-Node-Key header")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="X-Clearbox-Node-Key header is required",
            headers={"WWW-Authenticate": "X-Clearbox-Node-Key"},
        )

    if not validate_node_key(x_clearbox_node_key):
        # Log a partial key hint (first 4 chars only) for diagnostics
        hint = x_clearbox_node_key[:4] + "..." if len(x_clearbox_node_key) > 4 else "???"
