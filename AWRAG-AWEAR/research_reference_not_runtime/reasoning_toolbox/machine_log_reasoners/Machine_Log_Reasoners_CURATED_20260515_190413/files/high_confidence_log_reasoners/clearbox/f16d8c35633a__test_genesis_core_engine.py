"""Tests for core.genesis.engine — direct lookup and BM25 search."""

import pytest

pytest.importorskip("rank_bm25")

from core.genesis.config import EXPECTED_BLOCK_COUNT
from core.genesis.engine import CitationEngine
...
def test_direct_lookup_all_tags(engine):
    """Every configured G-ID resolves successfully."""
    for i in range(1, EXPECTED_BLOCK_COUNT + 1):
        tag = f"G-{i:04d}"
...
def test_search_limit_respected(engine):
    ...
    resp = engine.search("clearbox", limit=3)
    assert resp["ok"] is True
...
def test_search_filter_derived_false(engine):
    ...
    from core.genesis.parser import parse_corpus
    blocks = parse_corpus()
...
def test_health(engine):
    ...
    h = engine.health()
    assert h["ok"] is True
    assert h["blocks"] == EXPECTED_BLOCK_COUNT


# ── List all ──────────────────────────────────────────────────────────────────

def test_list_all(engine):
    """list_all returns every configured entry with required fields."""
    items = engine.list_all()
    assert len(items) == EXPECTED_BLOCK_COUNT
    for item in items:
        assert "tag" in item
...
def test_index_rebuild_trigger(tmp_path, monkeypatch):
    ...
    import json
    from core.genesis.config import BUILD_META_PATH, INDEX_DIR
    ...
    monkeypatch.setattr("core.genesis.indexer.BUILD_META_PATH", fake_meta_path)
    monkeypatch.setattr("core.genesis.indexer.INDEX_DIR", tmp_path)
    monkeypatch.setattr("core.genesis.indexer.CORPUS_INDEX_PATH", tmp_path / "corpus.index.json")
    monkeypatch.setattr("core.genesis.indexer.BM25_INDEX_PATH", tmp_path / "bm25.index.pkl")

    from core.genesis.indexer import load_or_build_index
    idx = load_or_build_index()

    # Verify rebuilt
    meta = json.loads(fake_meta_path.read_text())
    assert meta["block_count"] == EXPECTED_BLOCK_COUNT
