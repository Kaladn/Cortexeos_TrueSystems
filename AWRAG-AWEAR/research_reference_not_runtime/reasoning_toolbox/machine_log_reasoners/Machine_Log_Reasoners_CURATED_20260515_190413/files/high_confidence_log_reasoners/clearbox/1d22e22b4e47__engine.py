"""Clearbox Node Engine -- manages discovery, registration, and job distribution.
...
Phase 4:  mDNS auto-discovery — peers appear automatically via _clearbox._tcp.local.
"""
...
from clearbox_node.api.models import NodeInfo
from clearbox_node.core.identity import get_machine_id
...
class NodeEngine:
    ...
    def _init_persistence(self):
        ...
        try:
            from security.data_paths import CLEARBOX_NODE_DIR
            self._nodes_file = CLEARBOX_NODE_DIR / "nodes.json"
    ...
    def get_local_caps(self) -> dict:
        if self._caps is None:
            from clearbox_node.core.caps import get_local_caps
    ...
    def store_pairing_secret(self, node_id: str, secret_hex: str) -> None:
        ...
        pair_file = CLEARBOX_NODE_PAIRS_DIR / f"{node_id}.json"
    ...
    async def verify_pairing(self, node_id: str, secret_hex: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error("Invalid hex in pairing secret for node %s", node_id)
            return None
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            nonce = ch_data.get("nonce")
            if not nonce:
                logger.error("Challenge response missing 'nonce' for node %s", node_id)
                return None
            ...
            if not token or expires_in is None:
                logger.error("Session response missing token/expires_in for node %s", node_id)
                return None
    ...
    async def _ensure_session(self, node_id: str) -> str:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            nonce = ch_data.get("nonce")
            if not nonce:
                raise ValueError(f"Challenge response missing 'nonce' for node {node_id}")
            ...
            expires_in = data.get("expires_in")
            if not token or expires_in is None:
                raise ValueError(f"Session response missing token/expires_in for node {node_id}")
            expires = time.time() + expires_in
            self._sessions[node_id] = (token, expires)
            return token
    ...
    def _auth_headers(self, token: str) -> dict:
        ...
        return {"Authorization": f"Clearbox {token}"}

    # ── mDNS Discovery (Phase 4) ────────────────────────────

    def start_discovery(self, port: int = 5060) -> bool:
        """Start mDNS broadcast + browsing. Auto-registers discovered peers.

        Returns True if discovery started, False if zeroconf unavailable.
        """
        try:
            from clearbox_node.core.discovery import MeshDiscovery
        except ImportError:
            logger.warning("zeroconf not installed — mDNS discovery disabled")
            return False

        caps = self.get_local_caps()
        self._discovery = MeshDiscovery(machine_id=self.local_node_id, port=port)
        self._discovery.set_properties(
            hostname=caps.get("hostname", ""),
            version="0.6.0",
            caps={"cpu": caps.get("cpu_cores", 0),
                  "ram": caps.get("ram_gb", 0),
                  "gpu": caps.get("gpu_name") or "none"},
        )
        self._discovery.set_on_change(self._on_peer_change)
        try:
    ...
    def _on_peer_change(self) -> None:
        """Callback from MeshDiscovery when peers appear/disappear.

        Auto-registers new mDNS peers into the node registry so they show
        up in the UI without manual IP:port entry.
        """
        if not self._discovery:
            return
        for peer in self._discovery.peers():
            mid = peer.get("machine_id", "")
            if not mid or mid == self.local_node_id:
                continue
            if mid not in self._nodes:
                # Parse caps from the mDNS TXT record
                caps_raw = peer.get("caps", "{}")
                try:
                ...
                node = NodeInfo(
                    ...
                    ip=peer.get("ip", ""),
                    port=peer.get("port", 5060),
                    caps_summary=caps,
                    pairing_state="unpaired",
                    last_seen=peer.get("last_seen", time.time()),
                )
                self._nodes[mid] = node
                logger.info("Auto-registered mDNS peer: %s (%s) @ %s:%d",
                            mid, node.hostname, node.ip, node.port)
                self._persist()

    def discovered_peers(self) -> list:
        """Raw mDNS peer list (for /node/peers endpoint)."""
        if self._discovery:
    ...
    async def proxy_get_mode(self, node_id: str) -> dict:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if resp.status_code == 401:
                self._sessions.pop(node_id, None)
                raise ValueError(f"Session expired for node {node_id} — retry to re-authenticate")
            resp.raise_for_status()
            data = resp.json()
