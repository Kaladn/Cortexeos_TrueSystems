"""Clearbox AI Lexicon bridge integrating the production lexicon with 6-1-6 mapping."""
...
from .clearbox_gpu import DeviceInfo, compute_window_counts, select_device
from .binary_store import (
    LexiconStoreReader, LexiconRecord, LexiconStoreAppender,
    StoreHeader, _iso_to_us, enforce_hex_uniqueness,
)
...
try:
    ...
    logging.getLogger(__name__).warning(
        "Governed I/O unavailable — falling back to legacy writes: %s", _gov_err
    )
    _GOVERNED = False
...
# Smart-char folding: applied before anchor extraction so curly quotes, dashes,
# and invisible separators don't silently mangle words.
_SMART_CHARS = str.maketrans({
    "\u2018": "'",   # left single quotation mark
    "\u2019": "'",   # right single quotation mark / apostrophe substitute
...
class ClearboxLexiconBridge:
    ...
    def __init__(
        ...
        self.ignored_words_path = self.reports_root / "ignored_words.json"
        self.ignored_words: set = set()
        self.pending_words_path = self.reports_root / "pending_words.json"
        self.pending_words: list = []  # ordered list of {"word": str, "added_at": str}
        self.temp_checkouts_path = self.reports_root / "temp_checkouts.json"
        self.temp_checkouts: Dict[str, Dict[str, Any]] = {}  # norm_word -> {hex, doc_id, checked_out_at}
    ...
    @staticmethod
    def _infer_pack_from_path(file_path: Path) -> Optional[str]:
        parent = file_path.parent.name.lower()
        name = file_path.name.lower()
        if parent == "canonical" or name.startswith("canonical_"):
            return "canonical"
        if parent == "medical" or name.startswith("med_"):
            return "medical"
        if parent == "structural" or name.startswith("structural"):
            return "structural"
        if parent == "temp_pool" or name.startswith("pool_temp"):
            return "temp_pool"
        if parent == "spare_slots" or name.startswith("pool_"):
            return "spare_slots"
        return None

    def _load_spare_slots(self, data: list, file_path: Path) -> int:
        """Load a spare slot file. Returns count of entries loaded."""
        inferred_pack = self._infer_pack_from_path(file_path)
        count = 0
        for entry in data:
            if not isinstance(entry, dict):
                continue
            entry_payload = dict(entry)
            if inferred_pack and "pack" not in entry_payload:
                entry_payload["pack"] = inferred_pack

            hex_addr = entry_payload.get("hex")
            if not hex_addr:
                continue
            status = str(entry_payload.get("status", "AVAILABLE")).upper()
            word = entry_payload.get("word")
            ...
            self.slots_total += 1
            if status in ("ASSIGNED", "STRUCTURAL", "CHECKED_OUT") and norm_word:
                self.slots_assigned += 1
                self.word_index[norm_word] = hex_addr
                self.frequency[norm_word] = int(entry_payload.get("frequency", 0))
            else:
    ...
    # ------------------------------------------------------------------
    # Binary authority load path
    # ------------------------------------------------------------------
    def _load_from_binary(self) -> Dict[str, int]:
        """Load active lexicon entries from lexicon.bin (binary authority).

        Spare slots (AVAILABLE) are NOT loaded — they stay on disk in
        Spare_Slots/*.json and are pulled on-demand by _pop_spare_slot().
        """
        bin_dir = self.lexicon_root
        reader = LexiconStoreReader(bin_dir)
        header = reader.load()

        count_entries = 0
        for rec in reader.iter_records():
            norm = rec.word.lower() if rec.word else ""
            if not norm:
                continue  # skip wordless records (shouldn't be in active store)

            # Reconstruct the payload dict to preserve downstream JSON contract
            entry_payload = {
                "hex": rec.hex_addr,
                "symbol": rec.hex_addr,
                "binary": rec.binary_repr,
                "font_symbol": rec.font_symbol,
                "tone_signature": f"TONE_{rec.tone_sig}",
                "status": rec.status,
                "word": rec.word,
                "display": rec.display or rec.word,
                "pack": rec.pool,
            }
            if rec.mapped_at_us:
                from .binary_store import _us_to_iso
                entry_payload["mapped_at"] = _us_to_iso(rec.mapped_at_us)

            # Key entries by hex_addr — matches _load_spare_slots behavior.
            # Browse/search services do entries.get(hex_addr), not entries.get(norm).
            self.entries[rec.hex_addr] = LexiconEntry(
                word=rec.word,
                symbol=rec.hex_addr,
                payload=entry_payload,
                status=rec.status,
            )
            self.word_index[norm] = rec.hex_addr
            self.slots_total += 1
            self.slots_assigned += 1
            count_entries += 1

        reader.close()
        return {"entries": count_entries, "source": "binary"}

    def _persist_to_binary(self, hex_addr: str, entry: "LexiconEntry") -> None:
        """Append/update a single entry in the binary authority store.

        Called after every mutation that changes an active lexicon record.
        Only writes if lexicon.bin exists (binary authority is active).
        Also enforces the hex uniqueness invariant on spare pools.
        """
        bin_path = self.lexicon_root / "lexicon.bin"
        if not bin_path.exists():
            return  # binary authority not active, skip

        payload = entry.payload if isinstance(entry.payload, dict) else {}
        tone_str = payload.get("tone_signature", "TONE_0")
        try:
        ...
        record = LexiconRecord(
            hex_addr=hex_addr,
            word=entry.word or "",
            display=payload.get("display", entry.word or ""),
            status=entry.status,
            category="structural" if payload.get("pack") == "structural" else "content",
            pool=payload.get("pack", "canonical"),
            tone_sig=tone_sig,
            font_symbol=payload.get("font_symbol", ""),
            binary_repr=payload.get("binary", ""),
            mapped_at_us=_iso_to_us(payload.get("mapped_at", "")),
            updated_at_us=StoreHeader.now_us(),
        )

        appender = LexiconStoreAppender(self.lexicon_root)
        appender.append_record(record)

        # Enforce hex uniqueness: remove this hex from spare/temp pools if present
        enforce_hex_uniqueness(self.lexicon_root)

    def _remove_from_binary(self, hex_addr: str) -> None:
        """Remove a record from binary authority indexes.

        Called when a slot is returned to pool / cleared.
        Only acts if lexicon.bin exists.
        """
        bin_path = self.lexicon_root / "lexicon.bin"
        if not bin_path.exists():
            return
        ...
        appender = LexiconStoreAppender(self.lexicon_root)
        appender.remove_record(hex_addr)
    ...
    def load(self) -> Dict[str, int]:
        ...
        # ── Binary authority path ─────────────────────────────
        # If lexicon.bin exists, load active entries from binary.
        # Spare slots stay in JSON for on-demand allocation.
        bin_path = self.lexicon_root / "lexicon.bin"
        if bin_path.exists():
            LOGGER.info("Binary lexicon store found — loading from lexicon.bin")
            result = self._load_from_binary()
            count_entries = result["entries"]
            LOGGER.info("Binary load: %s active entries", count_entries)

            # Still load enrichment state if present
            es_path = self.lexicon_root / "enrichment_state.json"
            if es_path.exists():
                self._load_enrichment_state(es_path)

        else:
            # ── Legacy JSON path ──────────────────────────────
            LOGGER.info("No lexicon.bin found — falling back to JSON load")
            for file_path in self.lexicon_root.rglob("*.json"):
                ...
                for key, payload in items:
                    ...
                    if isinstance(payload, dict):
                        ...
                        if inferred_pack and "pack" not in entry_payload:
                            entry_payload["pack"] = inferred_pack
                        symbol = entry_payload.get("symbol") or entry_payload.get("hex")
                        frequency = entry_payload.get("frequency") or entry_payload.get("freq")
                        try:
                            freq_int = int(frequency)
                        except (TypeError, ValueError):
                            freq_int = 0
                        self.frequency[norm] = freq_int
                        if isinstance(entry_payload.get("context_before"), dict) or isinstance(entry_payload.get("context_after"), dict):
                            entry_payload.setdefault("context_before", entry_payload.get("context_before", {}))
                            entry_payload.setdefault("context_after", entry_payload.get("context_after", {}))
                    else:
                        entry_payload = {"value": payload}
                        if inferred_pack:
                            entry_payload["pack"] = inferred_pack
                    symbol = symbol or entry_payload.get("hex") or fnv1a_hex(norm)
                    entry_payload.setdefault("symbol", symbol)
                    entry_payload.setdefault("hex", symbol)
                    status = str(entry_payload.get("status", "ASSIGNED")).upper()
                    self.entries[symbol] = LexiconEntry(word=norm, symbol=symbol, payload=entry_payload, status=status)
                    self.word_index[norm] = symbol
                    count_entries += 1

        self._apply_custom_entries()
        self._load_ignored_words()
        self._load_pending_words()
        self._load_temp_checkouts()
        self._rebuild_vocab()
        self.loaded = True
    ...
    def _apply_custom_entries(self) -> None:
        ...
        for norm, payload in data.items():
            ...
            symbol = payload.get("symbol") or payload.get("payload", {}).get("hex") or fnv1a_hex(norm)
            entry_payload = dict(payload.get("payload", {}))
            entry_payload.setdefault("word", word)
            entry_payload.setdefault("symbol", symbol)
            entry_payload.setdefault("hex", symbol)
            status = str(payload.get("status", entry_payload.get("status", "CUSTOM"))).upper()
            self.entries[symbol] = LexiconEntry(word=norm, symbol=symbol, payload=entry_payload, status=status)
            self.word_index[norm] = symbol
    ...
    # ------------------------------------------------------------------
    # Ignored words (persistent deny list for unmatched review queue)
    # ------------------------------------------------------------------
    def _load_ignored_words(self) -> None:
        self.ignored_words = set()
        if not self.ignored_words_path.exists():
            return
        try:
            with open(self.ignored_words_path, "r", encoding="utf-8") as fh:
                data = json.load(fh)
            if isinstance(data, list):
                self.ignored_words = {str(w).lower() for w in data if w}
        except (OSError, json.JSONDecodeError) as exc:
            LOGGER.warning("Failed reading ignored words: %s", exc)

    def _save_ignored_words(self) -> None:
        self.ignored_words_path.parent.mkdir(parents=True, exist_ok=True)
        with open(self.ignored_words_path, "w", encoding="utf-8") as fh:
            json.dump(sorted(self.ignored_words), fh, ensure_ascii=False, indent=2)

    def ignore_word(self, word: str) -> None:
        norm = word.strip().lower()
        if not norm:
            return
        self.ignored_words.add(norm)
        self._save_ignored_words()

    def unignore_word(self, word: str) -> None:
        norm = word.strip().lower()
        self.ignored_words.discard(norm)
        self._save_ignored_words()

    def get_ignored_words(self) -> list:
        return sorted(self.ignored_words)

    # ------------------------------------------------------------------
    # Pending words (approved but awaiting final slot assignment)
    # ------------------------------------------------------------------
    def _load_pending_words(self) -> None:
        self.pending_words = []
        if not self.pending_words_path.exists():
            return
        try:
            with open(self.pending_words_path, "r", encoding="utf-8") as fh:
                data = json.load(fh)
            if isinstance(data, list):
                self.pending_words = [
                    e for e in data
            ...
            LOGGER.warning("Failed reading pending words: %s", exc)

    def _save_pending_words(self) -> None:
        self.pending_words_path.parent.mkdir(parents=True, exist_ok=True)
        with open(self.pending_words_path, "w", encoding="utf-8") as fh:
            json.dump(self.pending_words, fh, ensure_ascii=False, indent=2)

    def add_pending_word(self, word: str) -> dict:
        norm = word.strip().lower()
        if not norm:
            return {"word": word, "pending": False, "reason": "empty"}
        # Already in lexicon?
        if norm in self.word_index:
            return {"word": word, "pending": False, "reason": "already_in_lexicon"}
        # Already pending?
        if any(e["word"] == norm for e in self.pending_words):
            return {"word": word, "pending": True, "reason": "already_pending"}
        entry = {
            "word": norm,
            "display": word.strip(),
            "added_at": datetime.utcnow().isoformat() + "+00:00",
        }
        self.pending_words.append(entry)
        self._save_pending_words()
        return {"word": word, "pending": True}

    def remove_pending_word(self, word: str) -> None:
        norm = word.strip().lower()
        self.pending_words = [e for e in self.pending_words if e["word"] != norm]
        self._save_pending_words()

    def get_pending_words(self) -> list:
        return list(self.pending_words)

    def clear_pending_words(self) -> int:
    ...
    # ------------------------------------------------------------------
    # Temp pool checkout / return (ephemeral document symbolization)
    # ------------------------------------------------------------------
    def _load_temp_checkouts(self) -> None:
        self.temp_checkouts = {}
        if not self.temp_checkouts_path.exists():
            return
        try:
            with open(self.temp_checkouts_path, "r", encoding="utf-8") as fh:
                data = json.load(fh)
            if isinstance(data, dict):
                self.temp_checkouts = data
        except (OSError, json.JSONDecodeError) as exc:
            LOGGER.warning("Failed reading temp checkouts: %s", exc)

    def _save_temp_checkouts(self) -> None:
        self.temp_checkouts_path.parent.mkdir(parents=True, exist_ok=True)
        with open(self.temp_checkouts_path, "w", encoding="utf-8") as fh:
            json.dump(self.temp_checkouts, fh, ensure_ascii=False, indent=2)

    def checkout_temp_slot(self, word: str, doc_id: str = "") -> Dict[str, Any]:
        """Borrow a temp pool slot for ephemeral document symbolization."""
        norm = self.normalise_word(word)
        # Already checked out?
        if norm in self.temp_checkouts:
            return {"word": word, "hex": self.temp_checkouts[norm]["hex"],
                    "symbol": self.temp_checkouts[norm]["hex"], "already": True}
        # Already in canonical lexicon?
        if norm in self.word_index:
            return self.lookup(word)

        # Find first available TEMP_POOL slot
        available_hex = None
        for hex_addr, entry in self.entries.items():
            if entry.status == "AVAILABLE" and not entry.word:
                if isinstance(entry.payload, dict) and entry.payload.get("pack") == "temp_pool":
                    available_hex = hex_addr
                    break
        if available_hex is None:
            raise ValueError("No available temp pool slots remaining")

        entry = self.entries[available_hex]
        entry.word = norm
        entry.status = "CHECKED_OUT"
        if isinstance(entry.payload, dict):
            entry.payload["word"] = norm
            entry.payload["display"] = word.strip()
            entry.payload["status"] = "CHECKED_OUT"
            entry.payload["doc_id"] = doc_id
            entry.payload["checked_out_at"] = datetime.utcnow().isoformat() + "+00:00"

        self.word_index[norm] = available_hex
        self.temp_checkouts[norm] = {
            "hex": available_hex,
            "display": word.strip(),
            "doc_id": doc_id,
            "checked_out_at": datetime.utcnow().isoformat() + "+00:00",
        }
        self._save_temp_checkouts()
        return {"word": word, "hex": available_hex, "symbol": available_hex, "temp": True}

    def return_temp_slot(self, word: str) -> Dict[str, Any]:
        """Return a checked-out temp slot back to the pool."""
        norm = self.normalise_word(word)
        if norm not in self.temp_checkouts:
            return {"word": word, "returned": False, "reason": "not_checked_out"}
        checkout = self.temp_checkouts.pop(norm)
        hex_addr = checkout["hex"]
        if hex_addr in self.entries:
            entry = self.entries[hex_addr]
            entry.word = None
            entry.status = "AVAILABLE"
            if isinstance(entry.payload, dict):
                entry.payload["word"] = None
                entry.payload["display"] = None
                entry.payload["status"] = "AVAILABLE"
                entry.payload.pop("doc_id", None)
                entry.payload.pop("checked_out_at", None)
        if norm in self.word_index:
            del self.word_index[norm]
        self._save_temp_checkouts()
        return {"word": word, "hex": hex_addr, "returned": True}

    def return_all_temp_by_doc(self, doc_id: str) -> Dict[str, Any]:
        """Return all temp slots checked out for a specific document."""
        to_return = [w for w, co in self.temp_checkouts.items() if co.get("doc_id") == doc_id]
        results = []
        for w in to_return:
            results.append(self.return_temp_slot(w))
        return {"doc_id": doc_id, "returned": len(results), "results": results}

    def get_stale_temp_checkouts(self, stale_hours: int = 24) -> list:
        """Return temp checkouts older than stale_hours."""
        cutoff = datetime.utcnow().timestamp() - (stale_hours * 3600)
        stale = []
        for norm, co in self.temp_checkouts.items():
            checked_out_at = co.get("checked_out_at", "")
            try:
                ts = datetime.fromisoformat(checked_out_at.replace("+00:00", "")).timestamp()
            except (ValueError, AttributeError):
                ts = 0
            if ts < cutoff:
                stale.append({
                    "word": co.get("display", norm),
                    "hex": co.get("hex"),
                    "doc_id": co.get("doc_id", ""),
                    "checked_out_at": checked_out_at,
                    "hours_ago": round((datetime.utcnow().timestamp() - ts) / 3600, 1),
                })
        return stale

    def promote_temp_to_canonical(self, word: str) -> Dict[str, Any]:
        """Move a temp checkout to permanent canonical assignment."""
        norm = self.normalise_word(word)
        if norm not in self.temp_checkouts:
            raise ValueError(f"'{word}' is not checked out from temp pool")
        checkout = self.temp_checkouts.pop(norm)
        old_hex = checkout["hex"]

        # Return the temp slot
        if old_hex in self.entries:
            entry = self.entries[old_hex]
            entry.word = None
            entry.status = "AVAILABLE"
            if isinstance(entry.payload, dict):
                entry.payload["word"] = None
        ...
        # Now assign a canonical slot
        return self.assign_to_canonical_slot(word)
    ...
    # ------------------------------------------------------------------
    # Word helpers / filters
    # ------------------------------------------------------------------
    def normalise_word(self, word: str) -> str:
        return word.lower()

    def _get_entry(self, word: Optional[str] = None, hex_addr: Optional[str] = None) -> Optional[LexiconEntry]:
        if not hex_addr and word:
            hex_addr = self.word_index.get(self.normalise_word(word))
        return self.entries.get(hex_addr) if hex_addr else None

    def _ensure_vocab_index(self, word: str) -> int:
        norm = self.normalise_word(word)
    ...
    def _encode_words(self, tokens: Iterable[str]) -> List[int]:
        return [self._ensure_vocab_index(w) for w in tokens]

    def _decode_index(self, index: int) -> str:
        if index < 0 or index >= len(self._index_vocab):
            LOGGER.warning("_decode_index: index %d out of range (vocab size %d)", index, len(self._index_vocab))
            return f"<UNK:{index}>"
        return self._index_vocab[index]

    def _passes_filters(self, word: str) -> bool:
        if self.config.get("alpha_only") and not word.isalpha():
            return False
        if self._regex_include and not self._regex_include.search(word):
            return False
        if self._regex_exclude and self._regex_exclude.search(word):
            return False
        return True

    def extract_anchors(self, text: str) -> List[str]:
        """Extract anchor words from text. Words are words — no destruction."""
        text = unicodedata.normalize("NFKC", text)
        text = text.translate(_SMART_CHARS)
        words = text.lower().split()
        anchors = [w for w in words if len(w) >= self.min_len and self._passes_filters(w)]
        return anchors
                ...
                focus_pack[str(distance)] = [
                    {
                        ...
                        "symbol": self.word_index.get(token),
                    }
    ...
    def _attach_metadata(self, before_packed: Dict[str, Any], after_packed: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        ...
        for focus_word in focus_words:
            ...
            entry = self._get_entry(hex_addr=hex_addr)
            merged[focus_word] = {
    ...
    def map_words(self, tokens: List[str], source: str = "inline") -> Dict[str, Any]:
        if not self.loaded:
        ...
        token_ids = self._encode_words(tokens)
        start = time.time()
        ...
        report = {
            ...
            "window": self.window,
            "topK": self.top_k,
            "device_used": self.device_info.type if self.device_info else "cpu",
            "device_name": self.device_info.name if self.device_info else "CPU",
    ...
    def map_text(self, text: str, source: str = "inline", device: str = "cpu") -> Dict[str, Any]:
        """Map text with paragraph boundary enforcement.

        Double newline = wall.  No context crosses a paragraph break.
        Each paragraph is anchored and windowed independently.
        ...
        # Device override: "cpu" forces CPU even when GPU is available
        if device == "gpu" and self.device_info:
            _dev = self.device_info
        else:
            _dev = DeviceInfo(type="cpu", name="CPU")

        for para in paragraphs:
            tokens = self.extract_anchors(para)
            if not tokens:
                continue
            total_tokens += len(tokens)
            token_ids = self._encode_words(tokens)
    ...
    def _pop_spare_slot(self, word: str = "") -> Optional[Dict[str, Any]]:
        """Pull one AVAILABLE slot from the Spare_Slots directory on disk.

        Tries the letter-keyed pool first (pool_A.json for words starting with
        A, pool_B.json for B, etc.).  Falls back to pool_ext_* files if the
        letter pool is exhausted or the word starts with a non-alpha character.
        Removes the consumed entry from the file so it can never be reused.
        Returns None if no spare slots remain anywhere.
        """
        spare_dir = self.lexicon_root / "Spare_Slots"
        if not spare_dir.exists():
            return None

        # Build candidate file order: letter pool first, then ext pools
        first_char = word.strip()[0].upper() if word.strip() else ""
        candidate_files: list = []
        if first_char.isalpha():
            letter_file = spare_dir / f"pool_{first_char}.json"
            if letter_file.exists():
                candidate_files.append(letter_file)
        # Ext pools as fallback (sorted for determinism)
        candidate_files.extend(sorted(spare_dir.glob("pool_ext_*.json")))
        # Remaining letter pools as last resort
        candidate_files.extend(
            f for f in sorted(spare_dir.glob("pool_[A-Z].json"))
        ...
        for pool_file in candidate_files:
            try:
                with open(pool_file, "r", encoding="utf-8") as fh:
                    pool: list = json.load(fh)
            except (json.JSONDecodeError, OSError):
                continue
            if not isinstance(pool, list):
                continue
            for idx, slot in enumerate(pool):
                if isinstance(slot, dict) and str(slot.get("status", "")).upper() == "AVAILABLE" and not slot.get("word"):
                    pool.pop(idx)
                    with open(pool_file, "w", encoding="utf-8") as fh:
                        json.dump(pool, fh, ensure_ascii=False, indent=2)
                    LOGGER.info("Consumed spare slot %s from %s (word=%r)", slot.get("hex"), pool_file.name, word)
                    return slot
        return None

    def assign_to_canonical_slot(self, word: str) -> Dict[str, Any]:
        """Bind a single word to the first available canonical spare slot.

        Searches in-memory entries first, then falls back to pulling a slot
        directly from the Spare_Slots directory on disk.
        Raises ValueError if no spare slots remain anywhere.
        """
        norm = self.normalise_word(word)
        if norm in self.word_index:
            return self.lookup(word)  # already present — idempotent

        # Find first available slot already in memory
        available_hex = None
        for hex_addr, entry in self.entries.items():
            if entry.status == "AVAILABLE" and not entry.word:
                available_hex = hex_addr
        ...
        # Not in memory — pull one from disk Spare_Slots
        if available_hex is None:
            slot = self._pop_spare_slot(word)
            if slot is None:
                raise ValueError("No available canonical slots remaining")
            hex_addr = slot["hex"]
            self.entries[hex_addr] = LexiconEntry(
                word=None,
                symbol=hex_addr,
        ...
        mapped_at = datetime.utcnow().isoformat() + "+00:00"
        if isinstance(entry.payload, dict):
            entry.payload["word"] = norm
            entry.payload["display"] = display
            entry.payload["symbol"] = available_hex
            entry.payload["status"] = "ASSIGNED"
            entry.payload["mapped_at"] = mapped_at
            entry.payload["pack"] = "canonical"
        else:
            entry.payload = {
                "word": norm,
                "display": display,
                "symbol": available_hex,
                "status": "ASSIGNED",
                "mapped_at": mapped_at,
                "pack": "canonical",
            }

        # Index both lowercase and original case so lookups always hit
        self.word_index[norm] = available_hex
        if display != norm:
            self.word_index[display] = available_hex
        self.frequency[norm] = 0
        self.slots_assigned += 1
        self.slots_available -= 1

        self._write_canonical_files()
        self._persist_to_binary(available_hex, entry)
        return self.lookup(word)
    ...
    def _write_canonical_files(self) -> int:
        ...
        written = 0
        # lexicon_root IS the canonical dir (e.g. ./Lexical Data/Canonical)
        canonical_dir = self.lexicon_root
        canonical_dir.mkdir(parents=True, exist_ok=True)
        for letter, entries in by_letter.items():
            # Only write entries that have a word bound (ASSIGNED) — never write bare spare slots
            assigned = [e for e in entries if e.get("word") and e.get("status") == "ASSIGNED"]
            if not assigned:
                continue
    ...
    def clear_canonical(self) -> Dict[str, Any]:
        ...
        self.slots_assigned = sum(
            ...
            if e.status in ("ASSIGNED", "STRUCTURAL", "CHECKED_OUT") and e.word
        )
        ...
        # Remove from binary authority
        for hex_addr in purged_hexes:
            self._remove_from_binary(hex_addr)
    ...
    def rollback(self, snapshot_path: Path, *, persist: bool = True) -> Dict[str, Any]:
        ...
        for entry_key, payload in entries.items():
            entry_payload = dict(payload.get("payload", {}))
            symbol = payload.get("symbol") or entry_payload.get("hex") or entry_key
            word = payload.get("word")
            norm = word.strip().lower() if isinstance(word, str) and word.strip() else None
            status = str(payload.get("status", entry_payload.get("status", "AVAILABLE"))).upper()
            ...
            self.slots_total += 1
            if status in ("ASSIGNED", "STRUCTURAL", "CHECKED_OUT") and norm:
                self.word_index[norm] = symbol
                self.slots_assigned += 1
            else:
                self.slots_available += 1
    ...
    def write_report(self, name: str, payload: Dict[str, Any]) -> Path:
        ...
        folder = self.reports_root / "clearbox_616" / run_id
        folder.mkdir(parents=True, exist_ok=True)
...
def load_bridge_from_config(config_path: Path, *, load_lexicon: bool = True) -> ClearboxLexiconBridge:
...
__all__ = ["ClearboxLexiconBridge", "load_bridge_from_config"]
