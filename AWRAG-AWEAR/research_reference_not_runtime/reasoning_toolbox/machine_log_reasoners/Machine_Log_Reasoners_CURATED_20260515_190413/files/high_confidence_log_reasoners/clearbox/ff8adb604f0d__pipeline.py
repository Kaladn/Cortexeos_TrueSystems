...
def _store_chunks(
    ...
    _governed_write(f"chunks/{corpus_id}/{receipt_id}/relations.json", relations_content, _gw, _zone)
...
def rebuild_anchor_stats() -> Dict[str, Any]:
    ...
    for _corpus_id, receipt_dir in iter_receipt_dirs():
...
def ingest_text(
    ...
    # 3. Store
    lake_path = _store_chunks(resolved_corpus_id, receipt_id, chunks, all_anchors, all_relations)
