"""
Reasoning Engine Core - Non-LLM inference engine for 6-1-6 maps
...
Run: uvicorn core.reasoning_engine:app --port 5051 --reload
"""
import hashlib
import json
import logging
...
from security.data_paths import CHAT_MAPS_DIR, CLEARBOX_DATA_ROOT
...
# Cache directory
REASONING_CACHE_DIR = CLEARBOX_DATA_ROOT / "reasoning_cache"
...
class ReasoningEngine:
    ...
    def _iter_map_files(self):
        """Yield only reasoning map files, not auxiliary JSON."""
        for map_file in sorted(self.maps_dir.glob("*.json")):
            name = map_file.name.lower()
            if name.startswith("map_") or name == "616_map.json":
                yield map_file
    ...
    def _check_version(self):
        ...
        try:
            ...
            logger.warning("Unable to read reasoning version file %s: %s", VERSION_FILE, exc)
            return

        if current is not None:
            if current != CURRENT_VERSION:
                logger.warning("Version changed (%s to %s), rebuilding indexes", current, CURRENT_VERSION)
                self._rebuild_all()
        else:
        ...
        try:
            ...
            logger.warning("Unable to write reasoning version file %s: %s", VERSION_FILE, exc)
    ...
    def scan_and_update_indexes(self):
        ...
        current = {}
        for map_file in self._iter_map_files():
...
app = FastAPI(
    title="Clearbox AI Reasoning Engine",
...
async def startup_event():
    ...
    try:
        get_engine().scan_and_update_indexes()
...
async def rebuild_indexes():
    ...
    try:
        reasoning_engine = get_engine()
        reasoning_engine._rebuild_all()
        reasoning_engine.scan_and_update_indexes()
...
async def query(req: QueryRequest):
    ...
    try:
        ...
        if req.mode == "what_does_x_do":
            answer_frame = get_engine().query_what_does_x_do(subject, topk=req.topk)
        else:
...
async def get_stats():
    ...
    try:
        reasoning_engine = get_engine()
        with reasoning_engine._get_db_connection(MANIFEST_DB) as conn:
            n_docs = conn.execute("SELECT COUNT(*) FROM maps_manifest").fetchone()[0]

        with reasoning_engine._get_db_connection(TERM_STATS_DB) as conn:
            n_terms = conn.execute("SELECT COUNT(*) FROM term_stats").fetchone()[0]

        with reasoning_engine._get_db_connection(EDGE_SCORES_DB) as conn:
