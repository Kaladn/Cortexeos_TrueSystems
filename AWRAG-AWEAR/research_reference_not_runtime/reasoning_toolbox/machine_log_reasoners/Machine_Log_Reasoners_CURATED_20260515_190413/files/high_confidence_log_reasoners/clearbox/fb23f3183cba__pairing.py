...
from ..core.session_gate import require_hello_session
from ..core.usb_detect import detect_usb_devices, adb_push_file
from ..core.ed25519_auth import generate_keypair, MobileKeyStore

LOGGER = logging.getLogger("clearbox.node.pairing")

router = APIRouter(prefix="/pair", tags=["mobile-pairing"])
...
def _get_key_store() -> MobileKeyStore:
    ...
    if _key_store is None:
        ...
        _key_store = MobileKeyStore(CLEARBOX_NODE_MOBILE_KEYS)
    return _key_store
...
async def download_bundle(
    ...
    # Write to auth dir (not world-writable %TEMP%) with random filename
    try:
        from security.data_paths import AUTH_DIR as _enroll_dir
    except ImportError:
        _enroll_dir = Path(tempfile.gettempdir())
    _enroll_dir.mkdir(parents=True, exist_ok=True)
    tmp = tempfile.NamedTemporaryFile(
        mode="w", suffix=".json", prefix="clearbox_enroll_",
        delete=False, dir=str(_enroll_dir),
    )
...
async def adb_push_bundle(
    ...
    # Write bundle to auth dir with random filename (not world-writable %TEMP%)
    try:
        from security.data_paths import AUTH_DIR as _enroll_dir2
    except ImportError:
        _enroll_dir2 = Path(tempfile.gettempdir())
    _enroll_dir2.mkdir(parents=True, exist_ok=True)
    tmp_path = os.path.join(str(_enroll_dir2), f"clearbox_enroll_{secrets.token_hex(8)}.json")
    with open(tmp_path, "w") as f:
        json.dump(bundle, f, indent=2)

    # Push via ADB
    success = adb_push_file(serial, tmp_path, "/sdcard/Download/ClearboxAI/clearbox_enroll.json")
