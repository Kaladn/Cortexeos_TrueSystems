...
from bridges.binary_store import CitationStoreReader, CitationStoreAppender
...
# ── Binary citation store (data citations — INGEST/DOC coords) ─
_bin_reader: CitationStoreReader | None = None
_bin_appender: CitationStoreAppender | None = None


def _get_bin_reader() -> CitationStoreReader | None:
    """Lazy-load binary citation store reader. Returns None if store doesn't exist."""
    global _bin_reader
    if _bin_reader is not None:
        return _bin_reader
    bin_path = DATA_CITATIONS_BIN_DIR / "citations.bin"
    if not bin_path.exists():
        return None
    try:
        _bin_reader = CitationStoreReader(DATA_CITATIONS_BIN_DIR)
        _bin_reader.load()
        logger.info("Binary citation store loaded: %d records", _bin_reader.count())
        return _bin_reader
    except Exception as e:
        logger.warning("Failed to load binary citation store: %s", e)
        return None


def _get_bin_appender() -> CitationStoreAppender | None:
    """Lazy-load binary citation store appender. Returns None if store doesn't exist."""
    global _bin_appender
    if _bin_appender is not None:
        return _bin_appender
    bin_path = DATA_CITATIONS_BIN_DIR / "citations.bin"
    if not bin_path.exists():
        return None
    try:
        _bin_appender = CitationStoreAppender(DATA_CITATIONS_BIN_DIR)
        return _bin_appender
    except Exception as e:
        logger.warning("Failed to load binary citation appender: %s", e)
        return None
...
async def search_citations(req: CitationSearchRequest):
    ...
    try:
        ...
        # Also search binary store (data citations)
        reader = _get_bin_reader()
        if reader:
            for cite_id in reader._index:
                rec = reader.get(cite_id)
                if not rec:
                    continue
                haystack = " ".join(filter(None, [
                    rec.subject, rec.note, rec.coord,
                    rec.original_marker, rec.original_ref,
                ])).lower()
                score = sum(1 for t in query_lower if t in haystack)
                if score > 0:
                    results.append({
                        ...
                        "subject": rec.subject,
                        "note": rec.note,
                        "snippet": (rec.subject or rec.note or rec.coord)[:120],
                        "day": "binary",
                        "score": score,
                        "domain": "data",
                    })
...
async def resolve_citation(cite: Optional[str] = None, day: Optional[str] = None):
    ...
    if cite:
        ...
        # INGEST coord: resolve from binary citation store
        if cite.startswith("INGEST:"):
            reader = _get_bin_reader()
            if reader:
                rec = reader.resolve(cite)
                if rec:
                    return {
                        "cite": cite, "found": True,
                        "content": rec.original_ref or rec.note or "",
                        "source": rec.source,
                        "subject": rec.subject,
...
async def get_all_citations(
    ...
    limit: int = 5,
    domain: str = Query("all", regex="^(all|chat|data)$"),
):
    try:
        results = {"items": [], "cursor": None, "domain": domain}

        # Chat citations from JSON sidecars
        if domain in ("all", "chat"):
            chat_result = citation_store.get_all_paginated(cursor=cursor, limit=limit)
            results["items"].extend(chat_result.get("items", []))
            results["cursor"] = chat_result.get("cursor")
            results["chat_count"] = len(chat_result.get("items", []))

        # Data citations from binary store
        if domain in ("all", "data"):
            ...
            if reader:
                ...
                for cite_id in reader._index:
                    rec = reader.get(cite_id)
                    if rec:
                        data_items.append(rec.to_dict())
                data_items.sort(key=lambda x: x.get("created_at_utc", ""), reverse=True)
                data_items = data_items[:limit]
                results["items"].extend(data_items)
                results["data_count"] = len(data_items)
...
async def citations_distribution():
    try:
        dist = citation_store.distribution()
        # Add binary store count
        reader = _get_bin_reader()
        if reader:
            dist["data_citations"] = reader.count()
            dist["data_receipts"] = len(reader._rcpt_index)
        return dist
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def citations_health(days: Optional[int] = None):
    try:
        chat_health = citation_store.health_check(days=days)
        reader = _get_bin_reader()
        if reader:
            chat_health["binary_store"] = reader.health()
        return chat_health
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_citation(req: CitationDeleteRequest):
    try:
        ...
        # Try binary store first (data citations)
        reader = _get_bin_reader()
        if reader and reader.get(req.cite_id):
            appender = _get_bin_appender()
            if appender:
                appender.remove_record(req.cite_id)
                removed = True
                logger.info("Removed data citation %s from binary store", req.cite_id)

        # Try JSON sidecars (chat citations)
        if not removed:
            if req.hard:
