...
AWEAR retrieval does not perform pressure promotion. It may mark pressure
...
If AW found a candidate strongly, but qualification rejected it mostly because
...
its native rank key says AW found something strong
```
...
AWEAR retrieval itself may only mark:
...
```text
evidence packet -> pressure probe questions -> optional normal AW query/batch
```
