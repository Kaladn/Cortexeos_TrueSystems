# AWEAR Three-Lane Law

AWEAR is the evidence machine, not the thinker.
...
Thinking consumes AWEAR packets. It does not mutate intake, retrieval, speech,
native counts, ranking, citations, coordinates, or source datasets.

## Boundary Test

If a behavior changes which evidence is admitted during `awear query` or
`awear batch`, it belongs to retrieval only if it is mechanical filtering. If it
