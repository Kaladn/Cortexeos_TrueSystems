...
`src/awrag/engine/qualification.py` performed pressure promotion inside the
...
python -m pytest -q
python -m awear.cli status --runtime-root <runtime> --dataset <dataset>
python -m awear.cli system-metrics --runtime-root <runtime> --dataset-id <dataset> --repo <repo>
python -m awear.cli query --runtime-root <runtime> --dataset-id <dataset> --question "<dataset-local question>" --top-k 5
python -m awear.cli packet-speech --packet <fresh-query-packet> --out <runtime-report-folder>\packet_speech
python -m awear.cli batch --runtime-root <runtime> --dataset <dataset> --questions <questions.txt> --top-k 5 --workers 6 --no-progress
python -m awear.cli pressure-probe --batch-summary <fresh-batch-summary> --out <runtime-report-folder>\pressure_probe --max-questions-per-packet 2
```
