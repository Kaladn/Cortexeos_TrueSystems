# TrueCore Source Request

Location: `G:\AnchorWorks_reasoning_toolbox_pull_list\00_TRUECORE_REQUESTS_DO_NOT_RUN\request.md`

Purpose: identify the exact source/reference files Codex wants next for TrueCore temporal logging, event order, cognitive-auth, and logger foundation work.
...
Do not mutate TrueCore from these files automatically.
Use them as reference/mining material only.
Pattern -> clean TrueCore contract -> tests -> implementation.
```
...
This is TrueCore's basement layer.
```
...
Rewrite clean TrueCore temporal/event contracts.
...
It tells us what exists without dragging everything into TrueCore blindly.
```
...
risk
dependencies
recommended TrueCore destination
required tests
```
...
TrueCore runtime is current authority.
