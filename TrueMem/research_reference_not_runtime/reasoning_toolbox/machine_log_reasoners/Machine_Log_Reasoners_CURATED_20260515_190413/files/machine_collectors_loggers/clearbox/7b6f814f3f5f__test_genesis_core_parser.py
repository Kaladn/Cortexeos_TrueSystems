"""Tests for core.genesis.parser — corpus parsing."""

import pytest
from pathlib import Path

from core.genesis.parser import parse_corpus, _hash_body, _normalize_body
from core.genesis.config import CORPUS_PATH, EXPECTED_BLOCK_COUNT, REQUIRED_HEADER_FIELDS


# ── Corpus parse ─────────────────────────────────────────────────────────────

def test_parse_all_blocks():
    """All configured blocks parse cleanly with required fields present."""
    blocks = parse_corpus()
...
def test_tags_contiguous():
    """Tags are contiguous from G-0001 through the configured final block."""
    blocks = parse_corpus()
...
def test_corpus_path_is_repo_local():
    """Core Genesis must resolve its corpus from this repo, not a stale external path."""
    assert CORPUS_PATH.exists(), f"Corpus path missing: {CORPUS_PATH}"
    assert "clearbox-ai-studio-v2-main" in str(CORPUS_PATH).lower()
