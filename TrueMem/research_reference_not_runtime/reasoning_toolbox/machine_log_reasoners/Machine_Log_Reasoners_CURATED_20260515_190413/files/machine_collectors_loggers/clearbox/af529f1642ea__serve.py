"""Clearbox Network — local serve endpoints.
...
All endpoints require X-Clearbox-Node-Key header validated by auth.require_node_key.
...
Write safety:
    1. Write to <filename>.clearbox_tmp in same directory
...
Audit log:
    Every request (success or failure) emits one line to clearbox.network.audit logger.
...
LOGGER = logging.getLogger("clearbox.network.serve")
...
async def serve_write(
    ...
    Pipeline:
      ...
      2. Write upload to <name>.clearbox_tmp in the same directory
    ...
    tmp_path = resolved.parent / (resolved.name + ".clearbox_tmp")
    bytes_written = 0
