...
def make_router(state: BridgeState, ws_manager: WebSocketManager) -> APIRouter:
    ...
    # ── Dev Routing stubs (routing/ module removed) ──────────────
    from bridges.helpers import (
        load_routing_profile,
        validate_routing_profile as _rt_validate,
        list_named_profiles as _rt_list,
        load_named_profile as _rt_load_named,
        save_named_profile as _rt_save_named,
    )
    _RT_DEFAULTS: dict = {"name": "default", "pipeline": []}
    ...
    # ── /api/lakespeak/config ──────────────────────────────────

    @router.get("/api/lakespeak/config")
    ...
    @router.patch("/api/lakespeak/config")
