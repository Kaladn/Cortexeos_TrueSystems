"""Clearbox Node API Router -- FastAPI endpoints for distributed computing.

Mounted on the bridge server at /api/nodes.
All imports are lazy -- bridge server boots fine even if clearbox_node deps are missing.
...
router = APIRouter(prefix="/api/nodes", tags=["clearbox_node"])
...
@router.get("")
async def node_roster():
    """Normalized node roster for runtime and network surfaces."""
    try:
        from .. import VERSION
        from ..core.engine import HEARTBEAT_TIMEOUT_S
        ...
        nodes = [
            {
                "id": "this",
                "name": caps.get("hostname") or "This PC",
                "host": "localhost",
                "port": 5050,
                "ui_port": 8080,
                "online": True,
                "self": True,
                "version": VERSION,
                "entries": engine.node_count,
                "note": "Local controller node",
            }
        ]

        for node in engine.list_nodes():
            if not node.node_id or node.node_id == "this":
                continue
            online = bool(node.last_seen and (now - node.last_seen) <= HEARTBEAT_TIMEOUT_S)
            nodes.append(
                {
                    "id": node.node_id,
                    "name": node.hostname or node.node_id,
                    "host": node.ip or "unknown",
                    "port": node.port,
                    "ui_port": 8080,
                    "online": online,
                    "self": False,
        ...
        logger.error("Clearbox Node roster error: %s", e, exc_info=True)
        return {"nodes": [], "error": {"type": "roster_error", "message": str(e)}}
...
async def node_status():
    try:
        ...
        logger.error("Clearbox Node status error: %s", e, exc_info=True)
...
async def node_list():
    try:
        ...
        logger.error("Clearbox Node list error: %s", e, exc_info=True)
