...
class BridgeState:
    def __init__(self, config_path: Path):
        ...
        self.plugin_gateway = PluginGatewayService(state=self)
    ...
    def get_spellchecker(self):
        if self._spellchecker is None:
            self._spellchecker = self.plugin_gateway.create_spellchecker(
    ...
    def get_alias_map(self):
        if self._alias_map is None:
            self._alias_map = self.plugin_gateway.create_alias_map(
    ...
    def get_correction_queue(self):
        if self._correction_queue is None:
            self._correction_queue = self.plugin_gateway.create_correction_queue(
                data_dir=STATE_DIR / "spellcheck"
            )
