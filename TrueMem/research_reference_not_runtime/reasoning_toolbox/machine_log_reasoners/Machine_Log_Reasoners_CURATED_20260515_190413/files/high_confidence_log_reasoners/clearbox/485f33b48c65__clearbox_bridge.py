...
DEFAULT_TOKEN_SPLIT = r"[^A-Za-z0-9']+"  # kept for tests; tokenize() uses re.findall instead

# Smart-char folding: applied before tokenization so curly quotes, dashes,
...
class ClearboxLexiconBridge:
    ...
    def load(self) -> Dict[str, int]:
        ...
        for file_path in self.lexicon_root.rglob("*.json"):
            ...
            for key, payload in items:
                ...
                self.entries[norm] = LexiconEntry(word=word, symbol=symbol, payload=entry_payload, status=status)
    ...
    def tokenize(self, text: str) -> List[str]:
        # Contract A: Unicode-aware tokenization — non-ASCII word tokens must be
        # emitted so they appear as unmapped rather than silently discarded.
        text = unicodedata.normalize("NFKC", text)  # ﬁ→fi, ™→TM, fullwidth→ASCII
        text = text.translate(_SMART_CHARS)          # curly quotes, dashes, invisible chars
    ...
    def assign_to_canonical_slot(self, word: str) -> Dict[str, Any]:
        ...
        Raises ValueError if the word is already in the lexicon or if no
        spare slots remain.
        """
        norm = self.normalise_token(word)
        if norm in self.word_index:
        ...
        # Find first available slot (status == AVAILABLE, no word bound)
        available_hex = None
        for hex_addr, entry in self.entries.items():
            if entry.status == "AVAILABLE" and not entry.word:
                available_hex = hex_addr
    ...
    def _write_canonical_files(self) -> int:
        ...
        written = 0
        canonical_dir = self.lexicon_root / 'Canonical'
