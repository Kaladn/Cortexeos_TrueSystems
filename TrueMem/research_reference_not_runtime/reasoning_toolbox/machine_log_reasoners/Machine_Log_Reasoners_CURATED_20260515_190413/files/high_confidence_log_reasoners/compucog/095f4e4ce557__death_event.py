...
class DeathEventOperator:
    ...
    def analyze(self, seq: FrameSequence) -> Optional[OperatorResult]:
        ...
        # Confidence: insta-melt or excessive flinch indicates manipulation
        confidence = 0.0
        if is_instamelt:
            # TTD below threshold = high confidence of manipulation
            confidence = min(1.0, 1.0 - (time_to_death / self.instamelt_threshold))
        elif flinch_count > 10:  # Excessive flinch in 1 second
            confidence = min(1.0, flinch_count / 20.0)
        
        # Determine manipulation flags
        flags = []
        if is_instamelt:
            flags.append(ManipulationFlags.INSTA_MELT)
        if flinch_count > 10:
