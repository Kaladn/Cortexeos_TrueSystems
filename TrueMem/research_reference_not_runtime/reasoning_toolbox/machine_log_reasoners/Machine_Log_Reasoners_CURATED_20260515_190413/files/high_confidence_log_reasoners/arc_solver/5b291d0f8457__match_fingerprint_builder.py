...
class MatchFingerprintBuilder:
    ...
    def __init__(self):
        ...
        self.visual_stats = {name: {'sum': 0.0, 'sum_sq': 0.0, 'min': np.inf, 'max': -np.inf, 'values': []}
                            for name in self.VISUAL_FEATURES}
        self.gamepad_stats = {name: {'sum': 0.0, 'sum_sq': 0.0, 'min': np.inf, 'max': -np.inf, 'values': []}
                             for name in self.GAMEPAD_FEATURES}
        self.network_stats = {name: {'sum': 0.0, 'sum_sq': 0.0, 'min': np.inf, 'max': -np.inf, 'values': []}
                             for name in self.NETWORK_FEATURES}
        self.audio_stats = {name: {'sum': 0.0, 'sum_sq': 0.0, 'min': np.inf, 'max': -np.inf, 'values': []}
                           for name in self.AUDIO_FEATURES}
        
        # Cross-modal correlation accumulators
    ...
    def reset(self):
        """Reset for new match (optional - usually just create new instance)."""
        self.__init__()
