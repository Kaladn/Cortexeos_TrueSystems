...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="truemem-operator",
        description="Chat-first terminal operator shell for TrueMem command contracts.",
    )
    ...
    parser.add_argument("--runtime-root", type=Path, help="Runtime root for live TrueMem question mode.")
    parser.add_argument("--dataset-id", help="Dataset id for live TrueMem question mode.")
    parser.add_argument("--top-k", type=int, default=5, help="Top-K evidence locations for live TrueMem question mode.")
...
class OperatorShell:
    ...
    def run(self) -> None:
        ...
        if self.is_connected:
            ...
            print("No dataset connected. Start with --runtime-root and --dataset-id for TrueMem question mode.")
        print(render_menu())
    ...
    def _handle_menu_choice(self, choice: str, *, osrl_audit: dict[str, object]) -> dict[str, object]:
        ...
        if choice in {"3", "ingest", "ingest new dataset"}:
            return {"kind": "menu_card", "accepted": True, "mode": self.mode, "message": _command_card("docufilm-intake"), "osrl_audit": osrl_audit}
...
def render_menu() -> str:
    ...
    lines.extend(
        [
            ...
            "Heavy work is launched through the shown TrueMem CLI command cards.",
        ]
    )
    return "\n".join(lines)
...
def _open_dataset_card(shell: OperatorShell) -> str:
    lines = [
        ...
        "Relaunch with:",
        "python -m truemem.operator_shell --runtime-root <runtime> --dataset-id <dataset>",
    ]
...
def _overview_card() -> str:
    return "\n".join(
        [
            "Dataset overview",
            "run:",
            "python -m truemem.cli dataset-overview --runtime-root <runtime> --dataset <dataset> --out <overview-folder>",
...
def _export_card() -> str:
    return "\n".join(
        [
            ...
            "Q/A ledger:",
            "python -m truemem.cli qa-export --runtime-root <runtime> --dataset <dataset> --output <qa_export.jsonl>",
            "Receipts:",
