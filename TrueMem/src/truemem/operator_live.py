...
def run_live_shell(shell: OperatorShell, *, refresh_seconds: float = 10.0) -> None:
    """Run the optional Rich cockpit without changing TrueMem command behavior."""
    ...
    try:
        ...
        print("Rich is not installed. Falling back to the plain TrueMem operator shell.")
    ...
    with Live(render(), console=console, screen=False, refresh_per_second=1 / refresh_seconds) as live:
        while True:
            ...
            user_input = console.input("[bold cyan]truemem> [/]")
...
def _build_layout(
    *,
    ...
    title = Text(" TrueMem OPERATOR COCKPIT ", style="bold cyan")
