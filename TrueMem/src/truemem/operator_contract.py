...
COMMANDS: tuple[OperatorCommand, ...] = (
    OperatorCommand(
        ...
        cli_template="python -m truemem.cli docufilm-intake --runtime-root <runtime> --dataset-id <dataset> --source <source-folder> --output-format <split|native> --window 6 --workers auto --ram-budget-gb 8 --reserve-ram-fraction 0.15",
        touches=(
            ...
            "<runtime>/datasets/<dataset>/combined/ when native is selected",
        ),
        receipts=("dataset_manifest.json", "receipts/", "counts/*.awbin", "intake resource_plan", "optional verified native publication"),
        next_review=("status", "dataset_manifest.json", "split/native byte sizes", "native verification", "workers_actual", "production_ingest"),
        mutating=True,
    ),
    OperatorCommand(
        name="query",
