...
def build_citation_crosslinks(
    ...
    *,
    ...
    for left_row in left_rows:
        ...
        for right_row in right_rows:
            ...
            if len(shared) < min_shared:
                continue
            links.append({
                "schema": "truemem_citation_crosslink@1",
                ...
                "created_from": "truemem_dataset_local_query_packets",
    ...
    summary = with_protected_notice({
        "schema": "truemem_citation_crosslink_summary@1",
        "created_at": utc_now(),
...
def crosslink_anchor_set(text: str) -> set[str]:
    ...
    for anchor in anchorize(cleaned):
        if anchor in blocked:
            continue
        if anchor_kind(anchor) != "content" or len(anchor) < 3:
            continue
        if anchor.isdigit():
            continue
        out.add(anchor)
