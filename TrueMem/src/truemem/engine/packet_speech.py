...
from truemem.engine.evidence_formula import build_topk_pressure_trellis
from truemem.nlp_resolver import best_sentence


SCHEMA = "truemem_packet_speech_run@0"
EVIDENCE_SCHEMA = "truemem_packet_speech_evidence_trace@0"
PRETTY_SCHEMA = "truemem_packet_speech_pretty_answer@0"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        description="Report-only TrueMem packet speech. Reads existing query JSON and writes separate evidence/pretty outputs.",
    )
    parser.add_argument("--packet", action="append", type=Path, required=True, help="Existing TrueMem query result JSON. Repeatable.")
...
def run_packet_speech(*, packet_paths: list[Path], out_dir: Path) -> dict[str, Any]:
    ...
    _write_json(out_dir / "TRUEMEM_PACKET_SPEECH_SUMMARY.json", summary)
    _write_summary_markdown(out_dir / "TRUEMEM_PACKET_SPEECH_SUMMARY.md", summary)
    _write_json(out_dir / "receipts" / "run_receipt.json", {
        "schema": "truemem_packet_speech_run_receipt@0",
        "records_processed": len(records),
        "retrieval_ran": False,
        "topk_ran": False,
        "intake_ran": False,
    ...
    _write_json(out_dir / "receipts" / "inputs_receipt.json", {
        "schema": "truemem_packet_speech_inputs_receipt@0",
...
def build_packet_speech_record(*, packet: dict[str, Any], packet_path: Path, index: int) -> dict[str, Any]:
    ...
    evidence_trace = {
        ...
        "receipt": {
            ...
            "model_used": "none",
            "model_may_search": False,
            "evidence_authority": "truemem_locked_packet",
        },
    }
    pretty_answer = {
    ...
    return {
        "schema": "truemem_packet_speech_case@0",
        "case_id": case_id,
        "source_packet_path": str(packet_path),
...
def _select_answer_form(
    *,
    ...
    if required_terms and required_coverage >= 0.80 and not missing_values:
        reasons.append("TrueMem qualification receipts cover the required question terms")
        return "SUPPORTED_CLAIM", reasons
    if required_terms and missing_required_terms:
        reasons.append("TrueMem qualification receipts are missing required question terms")
...
def _render_pretty_answer(
    *,
    ...
    if selected_form == "NO_SUPPORT_FOUND":
        return (
            "TrueMem cannot support this from the admitted evidence packet. "
            f"Qualified cited locations: {location_count}. Reason: {reason_text}."
        )
    if selected_form == "WEAK_GENERIC_EVIDENCE":
        return (
            "TrueMem found cited text, but the evidence is too generic to state the answer cleanly. "
    ...
    if selected_form == "RELATED_BUT_UNSUPPORTED":
        return (
            "TrueMem found related cited evidence, but the packet does not support the full claim cleanly. "
            f"Selected speech path: {selected_path}. Document-only cited text: {document_answer} "
            f"Citations: {cite_text}. Reason: {reason_text}."
        )
    if selected_form == "SUPPORTED_CLAIM":
        return (
            f"TrueMem supports this from the cited packet. Selected speech path: {selected_path}. "
...
def _compare_artifact_states(before: list[ArtifactState], after: list[ArtifactState]) -> dict[str, Any]:
    ...
    return {
        "schema": "truemem_packet_speech_no_mutation_receipt@0",
...
def _write_markdown(path: Path, record: dict[str, Any]) -> None:
    ...
    lines = [
        f"# TrueMem Packet Speech {record['case_id']}",
        "",
