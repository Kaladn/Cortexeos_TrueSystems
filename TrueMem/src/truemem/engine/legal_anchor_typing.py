...
def classify_anchor_occurrences(text: str, *, window: int = 6) -> list[dict[str, Any]]:
    occurrences = _anchor_occurrences_with_shape(text)
    rows: list[dict[str, Any]] = []
    for index, occurrence in enumerate(occurrences):
        anchor = _clean_anchor(str(occurrence["text"]))
        if not anchor:
            continue
        left = [_clean_anchor(str(row["text"])) for row in occurrences[max(0, index - window) : index]]
        right = [_clean_anchor(str(row["text"])) for row in occurrences[index + 1 : index + 1 + window]]
        features = {
            ...
            "has_case_marker": _near_case_marker(occurrences, index, window),
            ...
            "punctuation_shape": occurrence["punctuation_shape"],
            "heading_shape": occurrence["heading_shape"],
        }
...
def typed_anchor_summary(text: str) -> dict[str, Any]:
    ...
    return {
        "schema": "truemem_typed_anchor_summary@1",
...
def _anchor_occurrences_with_shape(text: str) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
...
def _clean_anchor(value: str) -> str:
    cleaned = re.sub(r"(^[^\w§]+|[^\w§]+$)", "", value.casefold())
    if not cleaned:
        return ""
    if cleaned == "§":
        return cleaned
...
def _near_case_marker(occurrences: list[dict[str, Any]], index: int, window: int) -> bool:
    start = max(0, index - window)
    end = min(len(occurrences), index + window + 1)
    nearby = " ".join(str(row["text"]) for row in occurrences[start:end]).casefold()
