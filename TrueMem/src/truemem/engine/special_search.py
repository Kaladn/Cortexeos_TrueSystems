...
def special_search(
    ...
    *,
    ...
    This is read-only against TrueMem dataset artifacts. It writes receipts only
    ...
    block_anchor_cache = {block.block_ordinal: [str(anchor) for anchor in anchorize(block.text)] for block in blocks}
...
def _require_ready(paths: Any) -> None:
    ...
    if missing:
        raise FileNotFoundError("TrueMem dataset is missing required files: " + ", ".join(missing))
...
def _entry_anchor_stream(value: Any) -> list[str]:
    return [str(anchor) for anchor in anchorize(str(value or ""))]
...
def _search_solo_anchor(blocks: list[SearchBlock], trigger: dict[str, Any], anchor_cache: dict[int, list[str]], *, max_hits: int) -> list[SearchHit]:
    wanted = str(trigger.get("anchor") or "")
    if not wanted:
        return []
    hits = []
    for block in blocks:
...
def _write_summary(path: Path, *, trigger_count: int, unmatched_count: int, hit_count: int, expanded_count: int, mini_counts: dict[str, Any], graph: dict[str, Any], output_paths: dict[str, str], throughput_messages_per_second: float) -> Path:
    ...
    lines = [
        ...
        "",
        "JSON-list driven TrueMem special search. Candidate labels are not truth, diagnosis, or promotion.",
        "",
