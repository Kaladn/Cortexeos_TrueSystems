...
def create_handler(
    *,
    ...
    class TrueMemReadOnlyHandler(BaseHTTPRequestHandler):
        server_version = "TrueMemReadOnlyUI/0.05"
        ...
        def _handle_batch_run(self) -> None:
            ...
            if completed.returncode != 0:
                raise RuntimeError((completed.stderr or completed.stdout or "truemem batch failed").strip())
            stdout_lines = [line for line in completed.stdout.splitlines() if line.strip()]
            if not stdout_lines:
                raise RuntimeError("truemem batch produced no JSON summary")
            summary = json.loads(stdout_lines[-1])
            summary["schema"] = summary.get("schema", "truemem_batch_run_summary@1")
    ...
    return TrueMemReadOnlyHandler


def main() -> None:
    parser = argparse.ArgumentParser(description="Serve the optional read-only TrueMem UI.")
    parser.add_argument("--runtime-root", default=os.environ.get("TrueMem_RUNTIME_ROOT"), required=False)
    parser.add_argument("--dataset-id", default=os.environ.get("TrueMem_DATASET_ID"), required=False)
    parser.add_argument("--host", default=os.environ.get("TrueMem_UI_HOST", "127.0.0.1"))
    parser.add_argument("--port", type=int, default=int(os.environ.get("TrueMem_UI_PORT", "8765")))
    parser.add_argument("--static-root", default=os.environ.get("TrueMem_UI_STATIC_ROOT"))
    args = parser.parse_args()

    if not args.runtime_root:
        parser.error("--runtime-root or TrueMem_RUNTIME_ROOT is required")
    if not args.dataset_id:
        parser.error("--dataset-id or TrueMem_DATASET_ID is required")
    ...
    print(f"TrueMem read-only UI serving {url}")
