...
from truemem.engine.pressure_probe import clean_pressure_hypothesis_question, run_pressure_probe


def test_clean_pressure_hypothesis_question_strips_probe_meta_terms() -> None:
    question = "What nearby cited passage clarifies excuse, has, juror around [TMCIT-3db1fffb7b]?"

    cleaned = clean_pressure_hypothesis_question(question)

    assert cleaned == "excuse has juror"
    for meta in ("nearby", "cited", "passage", "clarifies", "around", "TMCIT"):
    ...
    path.write_text(
        json.dumps({
            ...
            "final_answer": {
                ...
                "text": "The court may excuse a juror. [TMCIT-1]",
                "citations": ["[TMCIT-1]"],
                "model_used": "none",
                "model_may_search": False,
            },
...
def test_pressure_probe_builds_bounded_followups_from_admitted_evidence(tmp_path: Path) -> None:
    ...
    assert result["schema"] == "truemem_pressure_probe_run@0"
    ...
    assert all(row["source_citation"] == "[TMCIT-1]" for row in questions)
    ...
    assert all("[TMCIT-" not in row["clean_question"] for row in questions)
    ...
    assert receipt["source"] == "existing_truemem_query_packets"
