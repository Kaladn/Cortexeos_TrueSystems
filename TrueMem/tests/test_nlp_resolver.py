...
from truemem.nlp_resolver import best_sentence, resolve_answer
...
def test_resolve_answer_does_not_render_abbreviation_fragment() -> None:
    ...
    assert answer["status"] == "answered_from_truemem_locations"
    assert "It is not necessary to excuse" in answer["text"]
    assert "e.g. a victim of a sexual offence" in answer["text"]
    assert " [TMCIT-eg]" in answer["text"]


def test_resolve_answer_renders_dataset_backed_greeting_as_crisp_reply() -> None:
    packet = {
        "locations": [
            {"citation": "[TMCIT-morning]", "text": "Morning! All good - given the marathon yesterday, good morning was implied."},
            {"citation": "[TMCIT-good]", "text": "oh! LOL. Good morning I forgot to say it."},
        ]
    }
    ...
    assert answer["citations"] == ["[TMCIT-morning]", "[TMCIT-good]"]
