...
TrueMachine is the Linux CompuCog cognition layer. SecureCore comes after it as
a consumer of its evidence; SecureCore is not part of this repository.
...
`--truevision-state`, `--trueaudio-state`, `--awrag-state`, and
`--awear-state` arguments. Each JSON/JSONL artifact must declare its own
