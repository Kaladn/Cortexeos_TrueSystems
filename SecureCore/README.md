...
SecureCore is clean-core only in this repo state.
...
- `securecore/help/` for static help corpus and code-index tooling
...
- `securecore/openai_session.py` for in-memory OpenAI API-key session handling
...
gated through SecureCore policy.
...
None of them exist as active SecureCore runtime services today.
```
...
- `python securecore\app.py` starts the localhost API/control backend on `127.0.0.1:5057`
