...
from securecore.routes.rbac import rbac_bp
from securecore.routes.events import events_bp
from securecore.routes.openai_session import openai_session_bp
from securecore.routes.ops import ops_bp
from securecore.email_sentinel.bridge import ImapHeaderBridge, StaticHeaderBridge
from securecore.email_sentinel.patterns import AlertPattern, DevicePool, UserDevice
from securecore.email_sentinel.routes import email_sentinel_bp, init_email_sentinel_routes
...
def _create_email_sentinel(data_dir: str) -> EmailSentinelService | None:
    ...
    try:
        ...
        poll_interval = int(os.environ.get("SECURECORE_EMAIL_POLL_SECONDS", "10"))
        if os.environ.get("SECURECORE_EMAIL_IMAP_ENABLED", "").lower() == "true":
            bridge = ImapHeaderBridge(
                host=os.environ["SECURECORE_EMAIL_IMAP_HOST"],
                username=os.environ["SECURECORE_EMAIL_IMAP_USER"],
                password=os.environ["SECURECORE_EMAIL_IMAP_PASS"],
                mailbox=os.environ.get("SECURECORE_EMAIL_IMAP_MAILBOX", "INBOX"),
                ssl=os.environ.get("SECURECORE_EMAIL_IMAP_SSL", "true").lower() != "false",
            )
        else:
        ...
        patterns = [
            AlertPattern(
                ...
                sender_domains=[
                    domain.strip().lower()
                    for domain in os.environ.get("SECURECORE_EMAIL_SECURITY_DOMAINS", "").split(",")
                    if domain.strip()
                ],
            ...
            AlertPattern(
                ...
                sender_domains=[
                    domain.strip().lower()
                    for domain in os.environ.get("SECURECORE_EMAIL_BANKING_DOMAINS", "").split(",")
                    if domain.strip()
                ],
...
def create_app() -> Flask:
    ...
    reaper = Reaper(
        ...
        policy=ReaperPolicy(
            ...
            auto_shun_enabled=_env_bool("SECURECORE_REAPER_AUTO_SHUN", False),
            auto_lock_enabled=_env_bool("SECURECORE_REAPER_AUTO_LOCK", False),
            auto_preserve_enabled=_env_bool("SECURECORE_REAPER_AUTO_PRESERVE", False),
            dry_run=_env_bool("SECURECORE_REAPER_DRY_RUN", True),
        ),
    )
    reaper.start()
