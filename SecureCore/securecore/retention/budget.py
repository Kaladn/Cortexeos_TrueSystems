"""Storage budget guard for SecureCore runtime detail data.

SecureCore keeps receipts and evidence references. Heavy native TrueVision or
TrueAudio state belongs to those systems, not to the SecureCore runtime log
budget.
"""
...
class StorageBudgetGuard:
    ...
    def _write_receipt(self, folder: str, stem: str, payload: dict[str, Any]) -> Path:
        ...
        body = {
            "schema_version": 1,
            "kind": f"securecore_storage_{stem}_receipt",
...
def _foreign_flag(path: Path) -> dict[str, str]:
    return {
        "path": str(path),
        "reason": "foreign_heavy_state_in_securecore_root",
