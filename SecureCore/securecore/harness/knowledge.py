"""Deterministic SecureCore knowledge and creator harness."""
...
class SecureCoreHarness:
    """Read-only creator/security harness for SecureCore.
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            "schema_version": 1,
            "kind": "securecore_creator_security_harness",
            "authority": {
                "llm_authority": False,
    ...
    def choose(self, request_text: str) -> dict[str, Any]:
        ...
        return {
            "schema_version": 1,
            "kind": "securecore_deterministic_harness_choice",
    ...
    def answer(self, request_text: str) -> dict[str, Any]:
        ...
        return {
            ...
            "metadata": {
                ...
                "source": "securecore_creator_security_harness",
            },
            "inference": {
                "model": "deterministic_securecore_harness",
                "llm_authority": False,
            },
        }
    ...
    def classify(self, request_text: str) -> HarnessIntent:
        ...
        return HarnessIntent("securecore_general", "securecore", ("temporal.read",), 0.55)

    def _system_map(self) -> dict[str, Any]:
        return {
            "core": sorted(_child_dirs(self.repo_root / "securecore")),
            "routes": sorted(path.stem for path in (self.repo_root / "securecore" / "routes").glob("*.py") if path.stem != "__init__"),
            "runtime": sorted(path.stem for path in (self.repo_root / "securecore" / "runtime").glob("*.py") if path.stem != "__init__"),
            "sensors": sorted(path.stem for path in (self.repo_root / "securecore" / "sensors").glob("*.py") if path.stem != "__init__"),
            "tools": sorted(path.stem for path in (self.repo_root / "securecore" / "tools").glob("*.py") if path.stem != "__init__"),
            "frontdoor": {
    ...
    def _agent_catalog(self) -> list[dict[str, Any]]:
        root = self.repo_root / "securecore" / "live_agents" / "AGENTS" / "agents"
        rows = []
    ...
    def _worker_candidates(self, intent: HarnessIntent) -> list[dict[str, Any]]:
        workers = self._worker_catalog()
        if intent.target_kind in {"worker", "runtime", "incident", "securecore"}:
            return workers
        return workers[:1]
    ...
    def _render_choice(self, choice: dict[str, Any]) -> str:
        lines = [
            f"SecureCore harness classified this as `{choice['intent']}`.",
