...
class SeedAdminEntrypointTests(unittest.TestCase):
            ...
            env.update(
                {
                    ...
                    "DATABASE_URL": f"sqlite:///{(tmp_path / 'securecore.db').as_posix()}",
                    "BIND_HOST": "127.0.0.1",
                    "BIND_PORT": "5057",
                    "HONEYPOT_ENABLED": "false",
                    "DATA_DIR": str(tmp_path / "data"),
                    "LOG_DIR": str(tmp_path / "logs"),
                    "SECURECORE_ADMIN_USER": "admin",
                    "SECURECORE_ADMIN_PASS": "test-admin-pass",
                }
            )
            env.pop("PYTHONPATH", None)

            result = subprocess.run(
                [sys.executable, "securecore\\cli\\seed_admin.py"],
                cwd=str(repo_root),
                env=env,
                capture_output=True,
                text=True,
    ...
    def test_seed_admin_can_reset_existing_admin_password(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            env.update(
                {
                    ...
                    "LOG_DIR": str(tmp_path / "logs"),
                    "SECURECORE_ADMIN_USER": "admin",
                    "SECURECORE_ADMIN_PASS": "first-pass",
                }
            )
            env.pop("PYTHONPATH", None)
            ...
            env["SECURECORE_ADMIN_PASS"] = "second-pass"
            env["SECURECORE_ADMIN_RESET"] = "true"
