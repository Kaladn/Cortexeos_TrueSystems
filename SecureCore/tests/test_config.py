...
class ConfigLoadTests(unittest.TestCase):
    def setUp(self):
        self.env_path = Path(__file__).resolve().parents[1] / "securecore" / ".env"
    ...
    def test_load_settings_reads_securecore_env_file(self):
        self.env_path.write_text(
            "\n".join(
                [
                    "SECRET_KEY=securecore-test-secret",
                    "JWT_SECRET_KEY=securecore-test-jwt-secret",
                    "DATABASE_URL=sqlite:///securecore-test.db",
                    "BIND_HOST=127.0.0.1",
                    "BIND_PORT=5057",
                ]
            ),
        ...
        command = (
            "from securecore.config import load_settings, validate_settings; "
            "s=load_settings(); "
            "assert s['SECRET_KEY'] == 'securecore-test-secret', s; "
            "assert s['JWT_SECRET_KEY'] == 'securecore-test-jwt-secret', s; "
            "validate_settings(s)"
        )
        env = os.environ.copy()
        for key in self.original_values:
