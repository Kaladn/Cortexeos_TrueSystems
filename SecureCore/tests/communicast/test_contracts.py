...
from securecore.communicast.contracts import (
...
class CommunicastContractTests(unittest.TestCase):
    ...
    def test_securecore_toolbox_result_can_target_anchorworks(self):
        message = build_communicast_message(
            message_id="msg-sc-toolbox-1",
            source_system="securecore",
            source_component="temporal_logger",
            target_system="anchorworks",
            topic="securecore.toolbox.result",
            authority="securecore_support",
            payload_schema="securecore.logger.summary.v1",
            payload={"healthy_loggers": 4, "written_streams": 4},
            evidence_refs=["forge:batch:logger-smoke-1"],
            trace_refs=["securecore:reader:logger-status-1"],
        )
    ...
    def test_securecore_cannot_claim_anchorworks_face_authority(self):
        message = build_communicast_message(
            ...
            topic="anchorworks.face.rendered",
            authority="aw_face",
            payload_schema="securecore.report.v1",
            payload={"text": "I am the face now."},
            evidence_refs=["forge:batch:1"],
            trace_refs=["securecore:writer:1"],
        )
    ...
    def test_messages_need_receipts(self):
        message = build_communicast_message(
            ...
            target_system="anchorworks",
            topic="securecore.agent.finding",
            authority="securecore_support",
            payload_schema="securecore.agent.finding.v1",
