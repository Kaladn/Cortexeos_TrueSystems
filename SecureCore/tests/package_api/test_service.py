...
class PackageServiceTests(unittest.TestCase):
    def test_status_summary_returns_facts_without_server_or_agents(self):
        ...
        self.assertIn("SecureCore package API is available.", response.facts)
    ...
    def test_fusion_latest_returns_latest_metadata_only(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            store.append(block)
            service = SecureCorePackageService(fusion_store=store)
    ...
    def test_runtime_status_route_uses_runtime_manager(self):
        ...
        service = SecureCorePackageService(runtime_manager=RuntimeManager())
        ...
        self.assertEqual(response.status, "ok")
        self.assertEqual(response.evidence_refs, ["securecore:runtime:status"])
        self.assertIn("Latest runtime run run-1 status is ok.", response.facts)

    def test_runtime_run_route_uses_bounded_runtime_manager(self):
        received_kwargs = {}

        class RuntimeManager:
            def run_logging_window(self, **kwargs):
                received_kwargs.update(kwargs)
                return {
                    "status": "ok",
                    "run_id": "run-2",
                    "report": {"fusion_block_events": 3},
                }
        ...
        response = service.handle(
            SecureCoreRequest(
                request_id="req-runtime-run",
                caller="anchorworks",
                route="runtime.logging.run",
                purpose="run bounded logging window",
                payload={
                    "duration_seconds": 1,
        ...
        self.assertEqual(response.status, "ok")
        self.assertEqual(response.evidence_refs, ["securecore:runtime:run:run-2"])
        self.assertIn("Runtime logging window run-2 completed with 3 fused events.", response.facts)
        self.assertTrue(received_kwargs["include_hid"])
        self.assertTrue(received_kwargs["include_vision"])
    ...
    def test_runtime_watch_latest_route_uses_runtime_manager(self):
        ...
        self.assertEqual(response.evidence_refs, ["securecore:runtime:watcher"])
