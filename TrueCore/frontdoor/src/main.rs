...
async fn main() {
    ...
    let frontend_port = env::var("FRONTDOOR_PORT")
        ...
        .unwrap_or(5050);
    let backend_base =
        env::var("TRUECORE_BACKEND").unwrap_or_else(|_| "http://127.0.0.1:5057".to_string());
    ...
    println!(
        "TrueCore front door listening on http://{} -> {}",
...
async fn no_ui_runtime() -> (StatusCode, &'static str) {
    (
        StatusCode::GONE,
        "TrueCore UI/chat runtime is not installed. Backend proxy and health endpoints remain available.",
    )
}
...
async fn proxy_api(
    ...
    let upstream = match builder.send().await {
        ...
        Err(error) => {
            return (
                ...
                format!("TrueCore backend unavailable: {error}"),
            )
                .into_response();
        }
    };
    ...
    let payload = match upstream.bytes().await {
        ...
        Err(error) => {
            return (
                StatusCode::BAD_GATEWAY,
                format!("TrueCore backend returned an unreadable response: {error}"),
            )
                .into_response();
        }
    };
