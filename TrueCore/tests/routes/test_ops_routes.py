...
class OpsRouteTests(unittest.TestCase):
    ...
    def test_toolbox_exposes_workers_loggers_and_routes_without_activation(self):
        ...
        self.assertEqual(payload["loggers"], [])
    ...
    def test_truevision_ops_tab_reports_external_authority_without_capture_or_activation(self):
        response = self.client.get(
            "/api/ops/truevision",
            headers={"Authorization": f"Bearer {self.admin_token}"},
        )
        ...
        payload = response.get_json()
        self.assertTrue(payload["ok"])
        self.assertEqual(payload["source_system"], "TrueVision")
        self.assertEqual(payload["intake_system"], "TrueVisionIntake")
        self.assertFalse(payload["capture_authorized"])
        self.assertFalse(payload["activation_authorized"])
        self.assertFalse(payload["mutation_authorized"])
        self.assertIn("raw_frames", payload["forbidden_source_truth"])
        self.assertIn("native TrueVision state artifacts", payload["source_truth"])

    def test_removed_runtime_routes_are_not_advertised(self):
        routes = {row["path"] for row in self.client.get(
            "/api/ops/toolbox",
            headers={"Authorization": f"Bearer {self.admin_token}"},
        ).get_json()["system_routes"]}
        self.assertNotIn("/api/ops/warmup", routes)
        self.assertNotIn("/api/ops/runtime/latest", routes)
