...
class ReaperConsensusTests(unittest.TestCase):
    def test_reaper_policy_defaults_are_safe(self):
        policy = ReaperPolicy()

        self.assertFalse(policy.auto_shun_enabled)
        self.assertFalse(policy.auto_lock_enabled)
        self.assertFalse(policy.auto_preserve_enabled)
    ...
    def test_reaper_executes_shun_when_consensus_is_actionable(self):
        ...
        try:
            ...
            hid.record_session_state(session_locked=False, idle_seconds=5, foreground_app="TrueCore")
            hid.record_keyboard_activity(key_event_count=5, typing_variance=0.2, active_window="TrueCore")

            reaper = Reaper(
                decisions_substrate=decisions,
                operator_writer=operator,
                hid_substrate=hid,
                policy=ReaperPolicy(min_confidence=0.7, auto_shun_enabled=True, dry_run=True),
            )

            with patch("truecore.control.reaper.is_shunned", return_value=False), patch(
                "truecore.control.reaper.shun_ip",
    ...
    def test_reaper_live_shun_requires_password_verification_ref(self):
        temp_dir = _make_repo_temp_dir()
        try:
            decisions = AgentDecisionsSubstrate(temp_dir)
            operator = OperatorSubstrate(temp_dir)
            ...
            reaper = Reaper(
                ...
                operator_writer=operator,
                hid_substrate=hid,
                policy=ReaperPolicy(min_confidence=0.7, auto_shun_enabled=True, dry_run=False),
            )

            with patch("truecore.control.reaper.is_shunned", return_value=False), patch(
                "truecore.control.reaper.shun_ip",
                return_value={"ok": True, "status": "shunned", "ip": "10.2.3.4", "firewall_rule_created": True},
            ) as shun_mock:
                reaper.start()
                decisions.record_decision(
                    agent_name="containment",
                    decision_type="shun_ip",
                    confidence=0.95,
                    cell_id="cell-password",
                    context={"ip": "10.2.3.4", "escalation_level": 4},
                )
                ...
                actions = operator.get_operator_history(limit=10)
                action_types = [record["payload"]["action"] for record in actions]
                self.assertIn("reaper_shun_password_required", action_types)

                reaper.stop()
        finally:
            shutil.rmtree(temp_dir, ignore_errors=True)

    def test_reaper_live_shun_accepts_password_verification_and_enforces_ten_second_cooldown(self):
        temp_dir = _make_repo_temp_dir()
        try:
            decisions = AgentDecisionsSubstrate(temp_dir)
            operator = OperatorSubstrate(temp_dir)
            ...
            with patch("truecore.control.reaper.is_shunned", return_value=False), patch(
                "truecore.control.reaper.shun_ip",
                return_value={"ok": True, "status": "shunned", "ip": "10.2.3.5", "firewall_rule_created": True},
            ) as shun_mock:
                reaper.start()
                decisions.record_decision(
                    agent_name="cognitive",
                    decision_type="cognitive_assessment",
                    confidence=0.95,
                    cell_id="cell-cooldown",
                    context={
                        "confidence": 0.95,
                        "threat_score": 0.95,
                        "human_likelihood": 0.05,
                ...
                context = {
                    ...
                    "password_verification_ref": "auth:password:verified:dec-1",
                    "decided_by_user": "lee",
                }
                decisions.record_decision(
                    agent_name="containment",
                ...
                self.assertEqual(shun_mock.call_count, 1)
                actions = operator.get_operator_history(limit=20)
                action_types = [record["payload"]["action"] for record in actions]
                self.assertIn("reaper_shun_execute", action_types)
                self.assertIn("reaper_action_cooldown", action_types)
