...
from truecore.retention.budget import StorageBudgetGuard, StorageBudgetPolicy
...
class StorageBudgetGuardTests(unittest.TestCase):
    ...
    def test_foreign_truevision_state_in_truecore_root_is_flagged_not_deleted(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            logs = root / "runtime" / "logs"
            logs.mkdir(parents=True)
            ...
            self.assertEqual(report["flags"][0]["reason"], "foreign_heavy_state_in_truecore_root")
