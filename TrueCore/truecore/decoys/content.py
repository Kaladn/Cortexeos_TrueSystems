...
<div class="header"><h1>TrueCore Admin</h1><span class="version">v{ver}</span></div>
...
def fake_user_database(cell_id: str) -> dict:
    ...
    for i in range(15):
        ...
        users.append({
            ...
            "email": f"{us[:8]}@internal.truecore.local",
...
def fake_config_dump(cell_id: str) -> dict:
    ...
    return {
        "database": {"host": "db-primary.internal.truecore.local", "port": 5432,
                      "username": "sc_app_user", "password": f"pg_{s[:24]}"},
        "redis": {"host": "cache.internal.truecore.local", "password": f"redis_{s[24:48]}"},
...
def fake_server_status(cell_id: str) -> dict:
    ...
    return {
        "hostname": "prod-web-01.truecore.local", "os": "Ubuntu 22.04.3 LTS",
...
def fake_network_map(cell_id: str) -> dict:
    return {
        ...
        ], "gateway": "10.0.0.1", "domain": "truecore.local",
    }
...
def fake_backup_listing(cell_id: str) -> dict:
    ...
    for i in range(8):
        ...
        backups.append({
            "filename": f"truecore_prod_2026-03-{(int(bs[:2],16)%28)+1:02d}_{bs[:8]}.sql.gz.enc",
            "size_mb": 500 + (int(bs[2:6], 16) % 4500), "encrypted": True,
            "checksum": f"sha256:{bs[:64]}",
        })
    return {"backup_location": "/mnt/backup/truecore/", "encryption": "AES-256-GCM", "backups": backups}
...
def fake_error_with_stack(cell_id: str, path: str) -> dict:
    ...
    return {
        ...
        "traceback": [
            f'  File "/opt/truecore/app/server.py", line {int(s[:3],16)%500}, in handle_request',
            f'  File "/opt/truecore/app/dispatch.py", line {int(s[3:6],16)%300}, in dispatch',
...
def fake_git_config() -> str:
    return (
        ...
        '[remote "origin"]\n\turl = git@github.com:truecore-internal/platform.git\n'
        "\tfetch = +refs/heads/*:refs/remotes/origin/*\n"
        '[user]\n\tname = deploy-bot\n\temail = deploy@truecore.local\n'
    )
