...
The Reaper can run:
  ...
  2. Standalone as a background daemon (python -m truecore.control.reaper)
...
class ReaperPolicy:
    ...
    def __init__(
        ...
        auto_shun_enabled: bool = False,
        auto_lock_enabled: bool = False,
        auto_preserve_enabled: bool = False,
        dry_run: bool = True,
        dangerous_action_cooldown_seconds: float = 10.0,
        password_verification_required_for_live_actions: bool = True,
    ):
        ...
        self.dangerous_action_cooldown_seconds = dangerous_action_cooldown_seconds
        self.password_verification_required_for_live_actions = password_verification_required_for_live_actions
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "dangerous_action_cooldown_seconds": self.dangerous_action_cooldown_seconds,
            "password_verification_required_for_live_actions": self.password_verification_required_for_live_actions,
        }
...
class Reaper:
    ...
    def __init__(
        ...
        self._dangerous_action_timestamps: dict[str, float] = {}
    ...
    def _execute_shun(self, cell_id: str, context: dict, confidence: float) -> None:
        ...
        if not self._dangerous_action_gate("shun_ip", ip, cell_id, context):
            return
    ...
    def _dangerous_action_gate(self, action: str, target: str, cell_id: str, context: dict) -> bool:
        """Gate live dangerous actions with cooldown and password-verification receipt."""
        key = f"{action}:{target}"
        now = time.time()
        cooldown = float(self._policy.dangerous_action_cooldown_seconds)
        last_action = self._dangerous_action_timestamps.get(key, 0)
        if now - last_action < cooldown:
            self._actions_skipped += 1
            self._record_operator_action(
                "reaper_action_cooldown",
                target,
                cell_id=cell_id,
                details=json.dumps({
                    "action": action,
                    "cooldown_seconds": cooldown,
                    "elapsed_seconds": round(now - last_action, 3),
                }),
            )
            return False

        if not self._policy.dry_run and self._policy.password_verification_required_for_live_actions:
            password_verified = context.get("password_verified") is True
            verification_ref = str(context.get("password_verification_ref", "")).strip()
            decided_by_user = str(context.get("decided_by_user", "")).strip()
            if not (password_verified and verification_ref and decided_by_user):
                self._actions_skipped += 1
                self._record_operator_action(
                    "reaper_shun_password_required" if action == "shun_ip" else "reaper_action_password_required",
                    target,
                    cell_id=cell_id,
                    details=json.dumps({
                        "action": action,
                        "password_verified": password_verified,
                        "password_verification_ref_present": bool(verification_ref),
                        "decided_by_user_present": bool(decided_by_user),
                    }),
                )
                return False

        self._dangerous_action_timestamps[key] = now
        return True
