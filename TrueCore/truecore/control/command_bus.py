"""Local command bus for TrueCore operator controls.
...
from truecore.control.shun import shun_ip, unshun_ip
...
class ControlBus:
    ...
    def start(self) -> None:
        if self._running:
            return
        self._running = True
        self._thread = threading.Thread(target=self._serve, daemon=True, name="truecore-control-bus")
        self._thread.start()
    ...
    def _dispatch(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        origin = self._verify_command_origin(command)

        if command == "status_snapshot":
            return {"snapshot": self._status_snapshot(), "command_origin": origin}
        if command == "reaper_pause":
            self._reaper.pause()
            return {"status": "paused", "command_origin": origin}
        if command == "reaper_resume":
            self._reaper.resume()
            return {"status": "resumed", "command_origin": origin}
        ...
        if command == "registry_snapshot":
            return {"registry": self._registry.summary() if self._registry else {}, "command_origin": origin}
        if command == "gate_denials":
            return {"denials": self._permission_gate.recent_denials() if self._permission_gate else [], "command_origin": origin}

        return {"ok": False, "error": f"unknown command: {command}"}

    def _verify_command_origin(self, command: str) -> dict[str, Any]:
        hid = self._substrates.get("hid") if isinstance(self._substrates, dict) else None
        if hid is None or not hasattr(hid, "get_recent_attestation"):
            raise ValueError("command requires active local HID attestation")
        attestation = hid.get_recent_attestation(window_seconds=30.0)
        return validate_hid_command_origin(str(command), attestation)
