...
AnchorWorks remains the user-facing system; TrueCore supplies support,
...
SYSTEMS = {"anchorworks", "truecore", "operator"}
TOPICS = {
    "anchorworks.task.request",
    "anchorworks.face.rendered",
    "truecore.agent.finding",
    "truecore.health",
    "truecore.security.event",
    "truecore.toolbox.capability",
    "truecore.toolbox.result",
}
AUTHORITIES = {
    "aw_face",
    "aw_task_conductor",
...
class CommunicastContractError(ValueError):
    """Raised when a Communicast message violates TrueMem/SC authority boundaries."""
...
def _validate_authority_boundary(message: dict[str, Any]) -> None:
    ...
    if topic == "anchorworks.face.rendered":
        ...
        return
    if source == "truecore" and authority == "aw_face":
        raise CommunicastContractError("TrueCore cannot claim AnchorWorks face authority")
    if source == "truecore" and message.get("user_facing") is True:
        raise CommunicastContractError("TrueCore support messages cannot be user-facing")
    if topic.startswith("truecore.") and source != "truecore":
        raise CommunicastContractError("truecore topics must originate from TrueCore")
