...
def _literal(node:ast.AST)->Any:
 try:
  def safe(value:Any)->Any:
   if isinstance(value,set):return {"set_members":[safe(x) for x in sorted(value,key=repr)]}
   if isinstance(value,tuple):return {"tuple_members":[safe(x) for x in value]}
   if isinstance(value,list):return [safe(x) for x in value]
   if isinstance(value,dict):return {str(k):safe(v) for k,v in value.items()}
   return value
  return safe(ast.literal_eval(node))
 ...
 for node in ast.walk(tree):
  ...
  for child in ast.walk(node):
   if isinstance(child,ast.Call):
    name=_name(child.func); entry={"name":name,"line":child.lineno,"arguments":[_literal(x) for x in child.args]};calls.append(entry)
...
def _local_maps(repo:Path)->tuple[list[dict],Counter,Counter,list[tuple]]:
 ...
 for source_path in LOCAL_FILES:
  raw=(repo/source_path).read_bytes();text=raw.decode("utf-8");paragraphs=[x for x in text.split("\n\n") if x.strip()]
  for paragraph in paragraphs:
   ...
   blocks.append({"block_ordinal":ordinal,"source_path":source_path,"source_sha256":digest(raw),"exact_text":paragraph,"exact_text_sha256":read_hash,"docufilm_read_hash":read_hash,"anchors":anchors})
...
def build_symbolic_adapter(curation_root:Path,source_root:Path,repo:Path,output:Path,workers:int=24,local_to_external:int=13)->dict:
 ...
 artifacts={"external_ast":write_jsonl(output/"external-networkx-ast.jsonl",external),"local_blocks":write_jsonl(output/"local-map/blocks.jsonl",blocks),"lexicon":write_json(output/"local-map/dataset_lexicon.json",{"schema":f"{SCHEMA}:isolated_six_byte_lexicon","symbol_bytes":6,"allocation":"sorted_frozen_isolated_curriculum_namespace","anchors":[{"anchor":a,"symbol":symbol_hex[a],"observed_count":counts[a]} for a in sorted(counts)]})}
 for key,name in (("anchor_binary","anchor_counts.awbin"),("relation_binary","relation_counts.awbin"),("posting_binary","block_anchor_postings.awbin")):
  raw=(output/"local-map"/name).read_bytes();artifacts[key]={"path":name,"bytes":len(raw),"sha256":digest(raw)}
 local_windows=[]
 for block in blocks:
  a=block["anchors"]
 ...
 artifacts["windows"]=write_jsonl(output/"local-map/windows-616.jsonl",local_windows);artifacts["schedule"]=write_jsonl(output/"curriculum-schedule.jsonl",schedule)
