"""Shared helpers for the TrueCore CLI command center."""
...
from truecore.config import load_settings
from truecore.forge.index import ForgeIndex
from truecore.forge.reader import ForgeReader
from truecore.substrates.base import SubstrateRecord
...
def truecore_root() -> Path:
    return Path(__file__).resolve().parent.parent


def data_dir() -> Path:
    return truecore_root() / _settings().get("DATA_DIR", "data")
...
def logs_dir() -> Path:
    return truecore_root() / _settings().get("LOG_DIR", "logs")


def forge_dir() -> Path:
    configured = os.getenv("TRUECORE_FORGE_DIR", "").strip()
    if configured:
