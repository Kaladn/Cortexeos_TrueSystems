...
Usage:
    truecore agents                    list all registered callers
    truecore agents inspect <id>       show full caller detail
    truecore agents permissions <id>   show permission map
    truecore agents denials            show recent permission denials
"""
...
from truecore.cli.common import request_live_command
...
def _list_agents() -> None:
    ...
    if not registry:
        print(f"  {_colorize('Live organism unavailable. Start TrueCore to see registered callers.', 'yellow')}")
        print()
        return
...
def _inspect(target: str) -> None:
    ...
    print(f"      truecore help show agents")
    agent_name = target.replace("agent:", "").replace("control:", "").replace("routes:", "")
    print(f"      truecore help where {agent_name}")
    print()
