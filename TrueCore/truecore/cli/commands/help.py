...
Usage:
    truecore help                         top-level nav
    truecore help search <query>          search corpus + code index
    truecore help show <help_id>          show help entry (default tier 1)
    truecore help show <help_id> --tier 3 explicit tier
    truecore help where <symbol>          code/file mapping
    truecore help doctor                  check for stale index / missing content
    truecore help sync                    rebuild code mirror index
"""
...
def _nav() -> None:
    from truecore.help.corpus import HelpCorpus
    corpus = HelpCorpus()
    entries = corpus.list_ids()

    print()
    print(_colorize("  TRUECORE HELP", "bold"))
...
def _search(query: str) -> None:
    from truecore.help.corpus import HelpCorpus
    from truecore.help.code_index import CodeMirrorIndex
...
def _show(help_id: str, tier: int) -> None:
    ...
    if not entry:
        ...
        print(f"  Try: truecore help search {help_id}\n")
        return
...
def _where(query: str) -> None:
    ...
    if not results:
        ...
        print(f"  Try: truecore help sync  (to rebuild the index)")
        print()
        return
...
def _doctor() -> None:
    ...
    for help_id in [e["help_id"] for e in corpus.list_ids()]:
        ...
        for cmd in t3.get("commands", []):
            # Extract the subcommand (e.g., "truecore reaper" -> "reaper")
