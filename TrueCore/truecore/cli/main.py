"""TrueCore CLI command center.

Operator surface for watching and controlling the organism.
CLI first. Metrics first. No browser dependency.

Usage:
    python -m truecore.cli.main status
    python -m truecore.cli.main tail ingress
    python -m truecore.cli.main tail evidence --cell abc123
    python -m truecore.cli.main cells
    python -m truecore.cli.main cells --cell abc123
    python -m truecore.cli.main reaper
    python -m truecore.cli.main reaper --pause
    python -m truecore.cli.main reaper --resume
    python -m truecore.cli.main forge --substrate ingress
    python -m truecore.cli.main forge --verify ingress
"""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="truecore",
        description="TrueCore CLI command center",
    )
...
def main() -> None:
    ...
    if args.command == "status":
        from truecore.cli.commands.status import run
        run()
    elif args.command == "tail":
        from truecore.cli.commands.tail import run
        run(args.target, args.n, args.cell, args.follow)
    elif args.command == "cells":
        from truecore.cli.commands.cells import run
        run(args.cell, args.locked, args.evidence)
    elif args.command == "reaper":
        from truecore.cli.commands.reaper import run
        run(args.pause, args.resume, args.shun, args.unshun)
    elif args.command == "forge":
        from truecore.cli.commands.forge import run
        run(args.substrate, args.verify, args.tail)
    elif args.command == "help":
        from truecore.cli.commands.help import run
        run(args.action, args.query, args.tier)
    elif args.command == "agents":
        from truecore.cli.commands.agents import run
